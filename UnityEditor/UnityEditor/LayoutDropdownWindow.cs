...
namespace UnityEditor
{
  internal class LayoutDropdownWindow : PopupWindowContent
  {
    ...
    {
      this.m_AnchorMin = so.FindProperty(nameof (m_AnchorMin));
      this.m_AnchorMax = so.FindProperty(nameof (m_AnchorMax));
    ...
    internal static void DrawLayoutModeHeadersOutsideRect(Rect rect, SerializedProperty anchorMin, SerializedProperty anchorMax, SerializedProperty position, SerializedProperty sizeDelta)
    {
      ...
      LayoutDropdownWindow.LayoutMode mode = LayoutDropdownWindow.SwappedVMode(LayoutDropdownWindow.GetLayoutModeForAxis(anchorMin, anchorMax, 1));
    ...
    private void TableGUI(Rect rect)
    {
      ...
      LayoutDropdownWindow.LayoutMode layoutMode1 = LayoutDropdownWindow.SwappedVMode(LayoutDropdownWindow.GetLayoutModeForAxis(this.m_AnchorMin, this.m_AnchorMax, 1));
    ...
    private static void SetLayoutModeForAxis(SerializedProperty anchorMin, int axis, LayoutDropdownWindow.LayoutMode layoutMode, bool doPivot, bool doPosition, Vector2[,] defaultValues)
    {
      ...
      for (int index = 0; index < anchorMin.serializedObject.targetObjects.Length; ++index)
      {
        ...
        if (doPosition)
        {
          ...
          {
            Vector2 sizeDelta = targetObject.sizeDelta;
            sizeDelta[axis] = 0.0f;
            targetObject.sizeDelta = sizeDelta;
          }
        }
      }
    ...
    {
      LayoutDropdownWindow.LayoutMode layoutModeForAxis = LayoutDropdownWindow.GetLayoutModeForAxis(anchorMin, anchorMax, 0);
      LayoutDropdownWindow.LayoutMode vMode = LayoutDropdownWindow.SwappedVMode(LayoutDropdownWindow.GetLayoutModeForAxis(anchorMin, anchorMax, 1));
