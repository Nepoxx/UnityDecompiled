...
namespace UnityEditor
{
  ...
  internal class OcclusionCullingWindow : EditorWindow
  {
    ...
    private void ModeToggle()
    {
      ...
      {
        this.m_Mode = (OcclusionCullingWindow.Mode) GUILayout.Toolbar((int) this.m_Mode, OcclusionCullingWindow.s_Styles.ModeToggles, (GUIStyle) "LargeButton", GUI.ToolbarButtonSize.FitToContents, new GUILayoutOption[0]);
    ...
    private bool ShowModePopup(Rect popupRect)
    {
      ...
      if (EditorGUI.DropdownButton(popupRect, new GUIContent(strArray[selectedIndex]), FocusType.Passive, EditorStyles.popup))
      {
    ...
    private void DisplayControls(UnityEngine.Object target, SceneView sceneView)
    {
      ...
      else
      {
        ...
        if (!flag1)
        {
          ...
          bool flag3 = GUI.Toggle(position, StaticOcclusionCullingVisualization.showVisibilityLines, "", GUIStyle.none);
          ...
          bool flag4 = GUI.Toggle(position, StaticOcclusionCullingVisualization.showPortals, "", GUIStyle.none);
        ...
        if (!flag1)
        {
          bool flag3 = GUILayout.Toggle(StaticOcclusionCullingVisualization.showGeometryCulling, "Occlusion culling");
          if (flag3 != StaticOcclusionCullingVisualization.showGeometryCulling)
          {
            StaticOcclusionCullingVisualization.showGeometryCulling = flag3;
            SceneView.RepaintAll();
          }
        }
      }
