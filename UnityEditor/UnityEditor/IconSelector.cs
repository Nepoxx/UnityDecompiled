...
namespace UnityEditor
{
  internal class IconSelector : EditorWindow
  {
    ...
    private static int s_HashIconSelector = nameof (IconSelector).GetHashCode();
    ...
    private void DoButton(GUIContent content, Texture2D selectedIcon, bool labelIcon)
    {
      ...
      if (!EditorGUILayout.IconButton(controlId, content, GUIStyle.none))
        return;
      Texture2D largeIcon = this.ConvertSmallIconToLargeIcon((Texture2D) content.image, labelIcon);
      Undo.RecordObject(this.m_TargetObject, "Set Icon On GameObject");
      EditorGUIUtility.SetIconForObject(this.m_TargetObject, largeIcon);
