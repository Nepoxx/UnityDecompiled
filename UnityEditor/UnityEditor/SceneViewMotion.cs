...
namespace UnityEditor
{
  internal class SceneViewMotion
  {
    ...
    public static void DoViewTool(SceneView view)
    {
      ...
      switch (typeForControl)
      {
        ...
        case EventType.Layout:
          ...
          break;
        case EventType.Used:
          if (GUIUtility.hotControl == viewToolId || SceneViewMotion.s_CurrentState == SceneViewMotion.MotionState.kInactive)
            break;
    ...
    private static void HandleMouseUp(SceneView view, int id, int button, int clickCount)
    {
      ...
      GUIUtility.hotControl = 0;
      RaycastHit hit;
      if (button == 2 && SceneViewMotion.s_CurrentState != SceneViewMotion.MotionState.kDragging && SceneViewMotion.RaycastWorld(Event.current.mousePosition, out hit))
      {
    ...
    private static bool RaycastWorld(Vector2 position, out RaycastHit hit)
    {
      ...
      if ((double) num == double.PositiveInfinity)
      {
        ...
        {
          RaycastHit hitInfo;
          if (componentsInChild.Raycast(worldRay, out hitInfo, float.PositiveInfinity) && (double) hitInfo.distance < (double) num)
          {
            hit = hitInfo;
            num = hit.distance;
          }
    ...
    private static void HandleMouseDrag(SceneView view, int id)
    {
      ...
      switch (Tools.s_LockedViewTool)
      {
        ...
        case ViewTool.Pan:
          ...
          view.FixNegativeSize();
          Vector3 vector3_1 = (Camera.current.ScreenToWorldPoint(view.camera.WorldToScreenPoint(view.pivot) + new Vector3(-Event.current.delta.x, Event.current.delta.y, 0.0f)) - view.pivot) * EditorGUIUtility.pixelsPerPoint;
          if (current.shift)
            vector3_1 *= 4f;
        ...
        case ViewTool.Zoom:
          ...
          if ((double) SceneViewMotion.s_TotalMotion < 0.0)
            view.size = SceneViewMotion.s_StartZoom * (float) (1.0 + (double) SceneViewMotion.s_TotalMotion * (1.0 / 1000.0));
          else
            view.size += (float) ((double) num * (double) SceneViewMotion.s_ZoomSpeed * (3.0 / 1000.0));
          break;
        case ViewTool.FPS:
          if (!view.in2DMode && !view.isRotationLocked)
          {
    ...
    private static void HandleKeyUp()
    {
      if (Tools.s_LockedViewTool != ViewTool.FPS)
        return;
      Event current = Event.current;
      if (current.keyCode == (Event) SceneViewMotion.kFPSForward.keyCode || current.keyCode == (Event) SceneViewMotion.kFPSBack.keyCode)
      {
        SceneViewMotion.s_Motion.z = 0.0f;
        current.Use();
      }
      else if (current.keyCode == (Event) SceneViewMotion.kFPSLeft.keyCode || current.keyCode == (Event) SceneViewMotion.kFPSRight.keyCode)
      {
        SceneViewMotion.s_Motion.x = 0.0f;
        current.Use();
      }
      else if (current.keyCode == (Event) SceneViewMotion.kFPSUp.keyCode || current.keyCode == (Event) SceneViewMotion.kFPSDown.keyCode)
      {
        SceneViewMotion.s_Motion.y = 0.0f;
        current.Use();
      }
