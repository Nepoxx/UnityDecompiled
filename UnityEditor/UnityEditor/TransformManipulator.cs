  ...
  internal class TransformManipulator
  {
    ...
    public static void SetResizeDelta(Vector3 scaleDelta, Vector3 pivotPosition, Quaternion pivotRotation)
    {
      ...
      for (int index = 0; index < TransformManipulator.s_MouseDownState.Length; ++index)
      {
        ...
        {
          Undo.RecordObject((Object) transformData.rectTransform, "Resize");
        }
        else
        {
          SpriteRenderer component = transformData.transform.GetComponent<SpriteRenderer>();
          if ((Object) component != (Object) null && component.drawMode != SpriteDrawMode.Simple)
            Undo.RecordObjects(new Object[2]
            {
              (Object) component,
              (Object) transformData.transform
            }, "Resize");
          else
            Undo.RecordObject((Object) transformData.transform, "Resize");
        }
      }
      for (int index = 0; index < TransformManipulator.s_MouseDownState.Length; ++index)
    ...
    {
      if (TransformManipulator.s_MouseDownState == null || (double) positionDelta.magnitude == 0.0)
        return;
      for (int index = 0; index < TransformManipulator.s_MouseDownState.Length; ++index)
      {
        TransformManipulator.TransformData transformData = TransformManipulator.s_MouseDownState[index];
      ...
      Vector3 positionDelta1 = TransformManipulator.s_MouseDownState[0].transform.position - TransformManipulator.s_MouseDownState[0].position;
      for (int index = 1; index < TransformManipulator.s_MouseDownState.Length; ++index)
        TransformManipulator.s_MouseDownState[index].SetPositionDelta(positionDelta1, false);
    }
    ...
    private struct TransformData
    {
      ...
      public static TransformManipulator.TransformData GetData(Transform t)
      {
        ...
        transformData.SetupTransformValues(t);
        ...
        else
        {
          ...
          if ((Object) component != (Object) null && component.drawMode != SpriteDrawMode.Simple)
            transformData.sizeDelta = component.size;
        }
      ...
      public void SetScaleDelta(Vector3 scaleDelta, Vector3 scalePivot, Quaternion scaleRotation, bool preferRectResize)
      {
        ...
        if (preferRectResize)
        {
          ...
          if ((Object) component != (Object) null && component.drawMode != SpriteDrawMode.Simple)
          {
            component.size = Vector2.Scale(this.sizeDelta, (Vector2) scaleDelta);
            return;
          }
        }
