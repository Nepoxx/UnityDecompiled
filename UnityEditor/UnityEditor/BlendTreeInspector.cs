...
namespace UnityEditor
{
  ...
  internal class BlendTreeInspector : Editor
  {
    ...
    internal static Action<UnityEditor.Animations.BlendTree> blendParameterInputChanged = (Action<UnityEditor.Animations.BlendTree>) null;
    ...
    private Texture2D m_BlendTex = (Texture2D) null;
    private List<Texture2D> m_WeightTexs = new List<Texture2D>();
    private string m_WarningMessage = (string) null;
    private GameObject m_VisInstance = (GameObject) null;
    ...
    {
      if (EditorApplication.isPlaying && (UnityEngine.Object) animator != (UnityEngine.Object) null && animator.enabled && animator.gameObject.activeInHierarchy)
      ...
      if (!((UnityEngine.Object) parentBlendTree != (UnityEngine.Object) null))
        return;
      ...
      if (EditorGUILayout.BeginFadeGroup(this.m_ShowGraph.faded))
      {
        ...
        else if (this.m_BlendType.intValue == 4)
        {
          for (int index = 0; index < this.m_BlendTree.recursiveBlendParameterCount; ++index)
          {
            ...
            EditorGUI.BeginChangeCheck();
            float parameterValue = EditorGUILayout.Slider(recursiveBlendParameter, BlendTreeInspector.GetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, recursiveBlendParameter), blendParameterMin, blendParameterMax, new GUILayoutOption[0]);
            if (EditorGUI.EndChangeCheck())
    ...
    private void ThresholdValues()
    {
      ...
      if (this.m_Childs.arraySize >= 2)
      {
        ...
        float floatValue3 = propertyRelative1.floatValue;
        float floatValue4 = propertyRelative2.floatValue;
        propertyRelative1.floatValue = num1;
        ...
        if (!this.m_UseAutomaticThresholds.boolValue)
        {
          ...
          {
            SerializedProperty propertyRelative3 = this.m_Childs.GetArrayElementAtIndex(index).FindPropertyRelative("m_Threshold");
            float t = Mathf.InverseLerp(floatValue3, floatValue4, propertyRelative3.floatValue);
            propertyRelative3.floatValue = Mathf.Lerp(num1, num2, t);
          }
        }
        float parameterValue = Mathf.Clamp(BlendTreeInspector.GetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, this.m_BlendTree.blendParameter), num1, num2);
        BlendTreeInspector.SetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, BlendTreeInspector.parentBlendTree, this.m_BlendTree.blendParameter, parameterValue);
      }
      this.m_MinThreshold.floatValue = num1;
      this.m_MaxThreshold.floatValue = num2;
    }
    ...
    private void BlendGraph(Rect area)
    {
      ...
      float num1 = Mathf.Min(numArray);
      float num2 = Mathf.Max(numArray);
      for (int index = 0; index < numArray.Length; ++index)
        numArray[index] = area.x + Mathf.InverseLerp(num1, num2, numArray[index]) * area.width;
      string blendParameter = this.m_BlendTree.blendParameter;
      Rect position = new Rect(area.x + Mathf.InverseLerp(num1, num2, BlendTreeInspector.GetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, blendParameter)) * area.width - 4f, area.y, 9f, 42f);
      Event current = Event.current;
      switch (current.GetTypeForControl(controlId))
      {
        case EventType.MouseDown:
          ...
          if (position.Contains(current.mousePosition))
          {
            ...
            num3 = BlendTreeInspector.GetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, blendParameter);
          }
          else if (area.Contains(current.mousePosition))
          {
            ...
            for (int index = 0; index < numArray.Length; ++index)
            {
              float num5 = index != 0 ? numArray[index - 1] : numArray[index];
              float num6 = index != numArray.Length - 1 ? numArray[index + 1] : numArray[index];
              if ((double) Mathf.Abs(x - numArray[index]) < (double) num4 && (double) x < (double) num6 && (double) x > (double) num5)
              {
                num4 = Mathf.Abs(x - numArray[index]);
                this.m_ReorderableList.index = index;
              }
            }
            this.m_UseAutomaticThresholds.boolValue = false;
            num3 = this.m_Childs.GetArrayElementAtIndex(this.m_ReorderableList.index).FindPropertyRelative("m_Threshold").floatValue;
          }
          float t = (current.mousePosition.x - area.x) / area.width;
          this.m_DragAndDropDelta = Mathf.LerpUnclamped(num1, num2, t) - num3;
          this.m_OriginMin = num1;
        ...
        case EventType.MouseDrag:
          if (GUIUtility.hotControl != controlId)
            break;
          current.Use();
          float num7 = Mathf.LerpUnclamped(this.m_OriginMin, this.m_OriginMax, (current.mousePosition.x - area.x) / area.width) - this.m_DragAndDropDelta;
          if (this.m_ReorderableList.index == -1)
          {
            float parameterValue = Mathf.Clamp(num7, num1, num2);
            BlendTreeInspector.SetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, BlendTreeInspector.parentBlendTree, blendParameter, parameterValue);
            break;
          }
          ...
          propertyRelative1.floatValue = num7;
          ...
          float num8 = (float) (3.0 * (((double) num2 - (double) num1) / (double) area.width));
          if ((double) propertyRelative1.floatValue - (double) propertyRelative2.floatValue <= (double) num8)
            propertyRelative1.floatValue = propertyRelative2.floatValue;
          else if ((double) propertyRelative3.floatValue - (double) propertyRelative1.floatValue <= (double) num8)
    ...
    private void BlendGraph2D(Rect area)
    {
      ...
      string blendParameter = this.m_BlendTree.blendParameter;
      string blendParameterY = this.m_BlendTree.blendParameterY;
      float parameterValue1 = BlendTreeInspector.GetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, blendParameter);
      float parameterValue2 = BlendTreeInspector.GetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, blendParameterY);
      ...
      BlendTreePreviewUtility.CalculateRootBlendTreeChildWeights(this.m_VisBlendTree.animator, 0, this.m_VisBlendTree.animator.GetCurrentAnimatorStateInfo(0).fullPathHash, this.m_Weights, parameterValue1, parameterValue2);
      Rect position1 = new Rect(area.x + Mathf.InverseLerp(vector2_1.x, vector2_2.x, parameterValue1) * area.width - 5f, area.y + (1f - Mathf.InverseLerp(vector2_1.y, vector2_2.y, parameterValue2)) * area.height - 5f, 11f, 11f);
      int controlId = GUIUtility.GetControlID(this.m_BlendAnimationID, FocusType.Passive);
      Event current = Event.current;
      switch (current.GetTypeForControl(controlId))
      {
        ...
        case EventType.MouseDrag:
          if (GUIUtility.hotControl == controlId)
          {
            if (this.m_SelectedPoint == -1)
            {
              ...
              BlendTreeInspector.SetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, BlendTreeInspector.parentBlendTree, blendParameter, vector2_3.x);
              BlendTreeInspector.SetParameterValue(BlendTreeInspector.currentAnimator, this.m_BlendTree, BlendTreeInspector.parentBlendTree, blendParameterY, vector2_3.y);
              current.Use();
              break;
            }
            for (int index = 0; index < motionPositions.Length; ++index)
        ...
        case EventType.Repaint:
          ...
          for (int index = 0; index < motionPositions.Length; ++index)
          {
            ...
            GUI.color = activeMotionIndices[index] >= 0 ? BlendTreeInspector.styles.visPointColor : BlendTreeInspector.styles.visPointEmptyColor;
    ...
    public void DrawChild(Rect r, int index, bool isActive, bool isFocused)
    {
      ...
      if (this.m_BlendType.intValue == 0)
      {
        ...
        using (new EditorGUI.DisabledScope(this.m_UseAutomaticThresholds.boolValue))
        {
          ...
          if (EditorGUI.EndChangeCheck())
          {
            if (float.TryParse(s, NumberStyles.Float, (IFormatProvider) CultureInfo.InvariantCulture.NumberFormat, out result))
            {
    ...
    private class Styles
    {
      ...
      public Styles()
      {
        ...
        this.visWeightLineColor = EditorGUIUtility.isProSkin ? new Color(1f, 1f, 1f, 0.6f) : new Color(0.0f, 0.0f, 0.0f, 0.3f);
        ...
        this.visPointOverlayColor = EditorGUIUtility.isProSkin ? new Color(1f, 1f, 1f, 0.4f) : new Color(0.0f, 0.0f, 0.0f, 0.2f);
