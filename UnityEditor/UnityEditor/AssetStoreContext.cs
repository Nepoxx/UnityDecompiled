  ...
  internal sealed class AssetStoreContext
  {
    ...
    public static AssetStoreContext GetInstance()
    {
      if (AssetStoreContext.s_Instance == null)
      {
        ...
        JSProxyMgr.GetInstance().AddGlobalObject(nameof (AssetStoreContext), (object) AssetStoreContext.s_Instance);
      }
    ...
    public static bool OpenPackageInternal(string id)
    {
      ...
      foreach (PackageInfo package in PackageInfo.GetPackageList())
      {
        if (package.jsonInfo != "")
        {
          ...
          string str = !jsonValue.Get(nameof (id)).IsNull() ? jsonValue[nameof (id)].AsString(true) : (string) null;
    ...
    public static void Download(string package_id, string url, string key, string package_name, string publisher_name, string category_name, AssetStoreUtils.DownloadDoneCallback doneCallback)
    {
      ...
      else
      {
        ...
        jsonValue2[nameof (url)] = (JSONValue) url;
        jsonValue2[nameof (key)] = (JSONValue) key;
    ...
    {
      string[] strArray = new string[3]{ publisher_name, category_name, package_name };
      for (int index = 0; index < 3; ++index)
        strArray[index] = AssetStoreContext.s_InvalidPathCharsRegExp.Replace(strArray[index], "");
      if (strArray[2] == "")
        strArray[2] = AssetStoreContext.s_InvalidPathCharsRegExp.Replace(package_id, "");
      if (strArray[2] == "")
        strArray[2] = AssetStoreContext.s_InvalidPathCharsRegExp.Replace(url, "");
      return strArray;
    }
    ...
    public AssetStoreContext.PackageList GetPackageList()
    {
      ...
      return new AssetStoreContext.PackageList() { results = array };
    }
