...
namespace UnityEditor
{
  internal class AudioMixerChannelStripView
  {
    ...
    private AudioMixerGroupController m_MovingSrcGroup = (AudioMixerGroupController) null;
    private AudioMixerGroupController m_MovingDstGroup = (AudioMixerGroupController) null;
    ...
    private bool ClipRect(Rect r, Rect clipRect, ref Rect overlap)
    {
      ...
      return (double) overlap.width > 0.0 && (double) overlap.height > 0.0;
    }
      ...
      switch (current.GetTypeForControl(this.m_EffectInteractionControlID))
      {
        ...
        case EventType.MouseDrag:
          ...
          if (this.HasFocus() && this.m_WaitingForDragEvent)
          {
            ...
            {
              if ((double) Mathf.Abs(current.delta.y) > (double) Mathf.Abs(current.delta.x))
              {
          ...
          if (this.IsAdjustingWetMix() && this.HasFocus() && showLevel)
          {
            ...
            if ((double) num != 0.0)
            {
              ...
              if (effect.IsSend() && this.m_Controller.CachedSelection.Count > 1 && this.m_Controller.CachedSelection.Contains(p.group))
              {
                ...
                foreach (AudioMixerGroupController mixerGroupController in this.m_Controller.CachedSelection)
                {
                  foreach (AudioMixerEffectController effect1 in mixerGroupController.effects)
                  {
                ...
                foreach (AudioMixerEffectController effectController in effectControllerList)
                {
                  if (!effectController.IsSend() || (UnityEngine.Object) effectController.sendTarget != (UnityEngine.Object) null)
                    effectController.SetValueForMixLevel(this.m_Controller, snapshot, Mathf.Clamp(effectController.GetValueForMixLevel(this.m_Controller, snapshot) + num, AudioMixerController.kMinVolume, AudioMixerController.kMaxEffect));
                }
              }
    ...
    private bool DoSoloButton(Rect r, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, List<AudioMixerGroupController> selection)
    {
      ...
      foreach (AudioMixerGroupController mixerGroupController in selection)
        mixerGroupController.solo = group.solo;
      return true;
    }
    ...
    private bool DoMuteButton(Rect r, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, bool anySoloActive, List<AudioMixerGroupController> selection)
    {
      ...
      foreach (AudioMixerGroupController mixerGroupController in selection)
        mixerGroupController.mute = group.mute;
      return true;
    }
    ...
    private bool DoBypassEffectsButton(Rect r, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, List<AudioMixerGroupController> selection)
    {
      ...
      foreach (AudioMixerGroupController mixerGroupController in selection)
        mixerGroupController.bypassEffects = group.bypassEffects;
      return true;
    }
    ...
    private static bool RectOverlaps(Rect r1, Rect r2)
    {
      ...
      return (double) rect.width > 0.0 && (double) rect.height > 0.0;
    }
    ...
    private void GroupClicked(AudioMixerGroupController clickedGroup, AudioMixerChannelStripView.ChannelStripParams p, bool clickedControlInGroup)
    {
      ...
      AudioMixerChannelStripView.\u003CGroupClicked\u003Ec__AnonStorey0 clickedCAnonStorey0 = new AudioMixerChannelStripView.\u003CGroupClicked\u003Ec__AnonStorey0();
      List<int> allInstanceIDs = new List<int>();
      foreach (AudioMixerGroupController shownGroup in p.shownGroups)
        allInstanceIDs.Add(shownGroup.GetInstanceID());
      List<int> selectedInstanceIDs = new List<int>();
      foreach (AudioMixerGroupController mixerGroupController in this.m_Controller.CachedSelection)
        selectedInstanceIDs.Add(mixerGroupController.GetInstanceID());
      ...
      clickedCAnonStorey0.newSelection = InternalEditorUtility.GetNewSelection(clickedGroup.GetInstanceID(), allInstanceIDs, selectedInstanceIDs, clickedInstanceId, keepMultiSelection, useShiftAsActionKey, allowMultiSelection);
      // ISSUE: reference to a compiler-generated method
      Selection.objects = (UnityEngine.Object[]) p.allGroups.Where<AudioMixerGroupController>(new Func<AudioMixerGroupController, bool>(clickedCAnonStorey0.\u003C\u003Em__0)).ToList<AudioMixerGroupController>().ToArray();
    ...
    private void DoAttenuationFader(Rect r, AudioMixerGroupController group, List<AudioMixerGroupController> selection, GUIStyle style)
    {
      ...
      foreach (AudioMixerGroupController mixerGroupController in selection)
      {
        float num4 = Mathf.Clamp(mixerGroupController.GetValueForVolume(this.m_Controller, this.m_Controller.TargetSnapshot), AudioMixerController.kMinVolume, AudioMixerController.GetMaxVolume());
        mixerGroupController.SetValueForVolume(this.m_Controller, this.m_Controller.TargetSnapshot, Mathf.Clamp(num4 + num3, AudioMixerController.kMinVolume, AudioMixerController.GetMaxVolume()));
      }
      InspectorWindow.RepaintAllInspectors();
    }
    ...
    internal static void AddEffectItemsToMenu(AudioMixerController controller, AudioMixerGroupController[] groups, int insertIndex, string prefix, GenericMenu pm)
    {
      ...
      for (int index = 0; index < effectList.Length; ++index)
      {
        if (effectList[index] != "Attenuation")
        {
          ...
          GUIContent content = new GUIContent(prefix + AudioMixerController.FixNameForPopupMenu(effectList[index]));
          int num = 0;
          // ISSUE: reference to a compiler-generated field
          ...
          // ISSUE: reference to a compiler-generated field
          GenericMenu.MenuFunction2 fMgCache0 = AudioMixerChannelStripView.\u003C\u003Ef__mg\u0024cache0;
          AudioMixerChannelStripView.EffectContext effectContext = new AudioMixerChannelStripView.EffectContext(controller, groups, insertIndex, effectList[index]);
          genericMenu.AddItem(content, num != 0, fMgCache0, (object) effectContext);
        }
      }
    }
    ...
    private void DoEffectSlotInsertEffectPopup(Rect buttonRect, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, int effectSlotIndex, ref Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap)
    {
      ...
      if (effectSlotIndex < group.effects.Length)
      {
        // ISSUE: object of a compiler-generated type is created
        // ISSUE: variable of a compiler-generated type
        AudioMixerChannelStripView.\u003CDoEffectSlotInsertEffectPopup\u003Ec__AnonStorey1 popupCAnonStorey1 = new AudioMixerChannelStripView.\u003CDoEffectSlotInsertEffectPopup\u003Ec__AnonStorey1();
        // ISSUE: reference to a compiler-generated field
        popupCAnonStorey1.\u0024this = this;
        // ISSUE: reference to a compiler-generated field
        popupCAnonStorey1.effect = group.effects[effectSlotIndex];
        // ISSUE: reference to a compiler-generated field
        // ISSUE: reference to a compiler-generated field
        // ISSUE: reference to a compiler-generated field
        // ISSUE: reference to a compiler-generated field
        if (!popupCAnonStorey1.effect.IsAttenuation() && !popupCAnonStorey1.effect.IsSend() && (!popupCAnonStorey1.effect.IsReceive() && !popupCAnonStorey1.effect.IsDuckVolume()))
        {
          // ISSUE: reference to a compiler-generated field
          // ISSUE: reference to a compiler-generated method
          pm.AddItem(new GUIContent("Allow Wet Mixing (causes higher memory usage)"), popupCAnonStorey1.effect.enableWetMix, new GenericMenu.MenuFunction(popupCAnonStorey1.\u003C\u003Em__0));
          // ISSUE: reference to a compiler-generated field
          // ISSUE: reference to a compiler-generated method
          pm.AddItem(new GUIContent("Bypass"), popupCAnonStorey1.effect.bypass, new GenericMenu.MenuFunction(popupCAnonStorey1.\u003C\u003Em__1));
          pm.AddSeparator("");
        }
        ...
        AudioMixerChannelStripView.AddEffectItemsToMenu(group.controller, groups, effectSlotIndex, "", pm);
      if (effectSlotIndex < group.effects.Length)
      {
        ...
        {
          pm.AddSeparator("");
          pm.AddItem(new GUIContent("Remove"), false, new GenericMenu.MenuFunction2(this.RemoveEffectPopupCallback), (object) new AudioMixerChannelStripView.EffectContext(this.m_Controller, groups, effectSlotIndex, ""));
          bool flag = false;
          if (effect.IsSend())
          {
            if ((UnityEngine.Object) effect.sendTarget != (UnityEngine.Object) null)
            {
              ...
              GenericMenu genericMenu = pm;
              GUIContent content = new GUIContent("Disconnect from '" + effect.GetSendTargetDisplayString(effectMap) + "'");
              int num = 0;
              // ISSUE: reference to a compiler-generated field
              ...
              // ISSUE: reference to a compiler-generated field
              GenericMenu.MenuFunction2 fMgCache1 = AudioMixerChannelStripView.\u003C\u003Ef__mg\u0024cache1;
              ...
              genericMenu.AddItem(content, num != 0, fMgCache1, (object) connectSendContext);
            }
            if (!flag)
    ...
    private void AddSeperatorIfAnyReturns(GenericMenu pm, List<AudioMixerGroupController> allGroups)
    {
      foreach (AudioMixerGroupController allGroup in allGroups)
      {
        foreach (AudioMixerEffectController effect in allGroup.effects)
        {
    ...
    public static void AddMenuItemsForReturns(GenericMenu pm, string prefix, int effectIndex, AudioMixerGroupController group, List<AudioMixerGroupController> allGroups, Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap, AudioMixerEffectController effect, bool showCurrent)
    {
      foreach (AudioMixerGroupController allGroup in allGroups)
      {
        foreach (AudioMixerEffectController effect1 in allGroup.effects)
        {
          if (MixerEffectDefinitions.EffectCanBeSidechainTarget(effect1))
          {
            ...
            if (!AudioMixerController.WillChangeOfEffectTargetCauseFeedback(allGroups, group, effectIndex, effect1, identifiedLoop))
            {
              ...
              {
                GenericMenu genericMenu = pm;
                GUIContent content = new GUIContent(prefix + "'" + effect1.GetDisplayString(effectMap) + "'");
                int num = (UnityEngine.Object) effect.sendTarget == (UnityEngine.Object) effect1 ? 1 : 0;
                // ISSUE: reference to a compiler-generated field
                ...
                // ISSUE: reference to a compiler-generated field
                GenericMenu.MenuFunction2 fMgCache2 = AudioMixerChannelStripView.\u003C\u003Ef__mg\u0024cache2;
                AudioMixerChannelStripView.ConnectSendContext connectSendContext = new AudioMixerChannelStripView.ConnectSendContext(effect, effect1);
                genericMenu.AddItem(content, num != 0, fMgCache2, (object) connectSendContext);
              }
            }
            else
            {
              ...
              int num = 1;
              foreach (AudioMixerController.ConnectionNode connectionNode in identifiedLoop)
              {
                pm.AddDisabledItem(new GUIContent(str + (object) num + ": " + connectionNode.GetDisplayString() + "->"));
                ++num;
              }
    ...
    public void OnGUI(Rect rect, bool showReferencedBuses, bool showBusConnections, bool showBusConnectionsOfSelection, List<AudioMixerGroupController> allGroups, Dictionary<AudioMixerEffectController, AudioMixerGroupController> effectMap, bool sortGroupsAlphabetically, bool showDeveloperOverlays, AudioMixerGroupController scrollToItem)
    {
      ...
      else
      {
        ...
        foreach (AudioMixerGroupController mixerGroupController in list1)
        {
          foreach (AudioMixerEffectController effect1 in mixerGroupController.effects)
          {
        ...
        int num3 = 1;
        foreach (AudioMixerGroupController mixerGroupController in list1)
          num3 = Mathf.Max(num3, mixerGroupController.effects.Length);
        ...
        for (int index = 0; index < list1.Count; ++index)
        {
          ...
          if (current.type == EventType.MouseDown && current.button == 0 && channelStripParams.stripRect.Contains(current.mousePosition))
            current.Use();
          if (this.IsMovingEffect() && current.type == EventType.MouseDrag && (channelStripParams.stripRect.Contains(current.mousePosition) && GUIUtility.hotControl == this.m_EffectInteractionControlID))
          {
            this.m_MovingEffectDstIndex = -1;
            current.Use();
          }
        ...
        {
          foreach (KeyValuePair<AudioMixerEffectController, AudioMixerChannelStripView.PatchSlot> keyValuePair in patchslots)
          {
            AudioMixerChannelStripView.PatchSlot patchSlot = keyValuePair.Value;
          ...
          foreach (KeyValuePair<AudioMixerEffectController, AudioMixerChannelStripView.PatchSlot> keyValuePair in patchslots)
          {
            AudioMixerEffectController key = keyValuePair.Key;
            AudioMixerEffectController sendTarget = key.sendTarget;
            if (!((UnityEngine.Object) sendTarget == (UnityEngine.Object) null))
            {
              AudioMixerChannelStripView.PatchSlot patchSlot1 = keyValuePair.Value;
    ...
    private void RectSelection(AudioMixerChannelStripView.ChannelStripParams channelStripParams)
    {
      ...
      {
        List<AudioMixerGroupController> mixerGroupControllerList = !current.shift ? new List<AudioMixerGroupController>() : this.m_Controller.CachedSelection;
        foreach (AudioMixerGroupController rectSelectionGroup in channelStripParams.rectSelectionGroups)
        {
          if (!mixerGroupControllerList.Contains(rectSelectionGroup))
            mixerGroupControllerList.Add(rectSelectionGroup);
        }
    ...
    private void WarningOverlay(List<AudioMixerGroupController> allGroups, Rect rect, Rect contentRect)
    {
      ...
      foreach (AudioMixerGroupController allGroup in allGroups)
      {
        ...
        if (allGroup.mute)
          ++num2;
        if (allGroup.bypassEffects)
          num3 += allGroup.effects.Length - 1;
        else
          num3 += ((IEnumerable<AudioMixerEffectController>) allGroup.effects).Count<AudioMixerEffectController>((Func<AudioMixerEffectController, bool>) (e => e.bypass));
      }
    ...
    private class ChannelStripParams
    {
      ...
      private List<Rect> GetBackgroundRects(Rect r, AudioMixerGroupController group, int maxNumGroups)
      {
        ...
        int num1 = maxNumGroups;
        for (int index = 0; index < num1; ++index)
          floatList.Add(16f);
        floatList.Add(10f);
        List<Rect> rectList = new List<Rect>();
        float y1 = r.y;
        foreach (int num2 in floatList)
        {
          if (rectList.Count > 0)
            y1 = y1;
          rectList.Add(new Rect(r.x, y1, r.width, (float) num2));
          y1 += (float) num2;
        }
        float y2 = y1 + 10f;
