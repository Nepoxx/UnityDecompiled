...
namespace UnityEditor
{
  ...
  internal class AssetStoreAssetInspector : Editor
  {
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (firstAsset != null)
      {
        ...
        if (firstAsset.className == null)
          ...
          str = firstAsset.className.Split(new char[1]
          {
            ' '
          }, 2)[0];
        string label2 = str;
        ...
        if (this.packageInfoShown)
        {
          ...
          EditorGUILayout.LabelField("Price", previewInfo != null ? (firstAsset.price == null || !(firstAsset.price != "") ? "free" : firstAsset.price) : "-", new GUILayoutOption[0]);
    ...
    public static void OpenItemInAssetStore(AssetStoreAsset activeAsset)
    {
      ...
      UsabilityAnalytics.Track(string.Format("/AssetStore/ViewInStore/{0}/{1}", (object) activeAsset.packageID, (object) activeAsset.id));
    }
    ...
    {
      return this.target != (Object) null && AssetStoreAssetSelection.Count != 0;
    }
    ...
    private void InitiateBuySelected(bool firstAttempt)
    {
      ...
      {
        switch (AssetStoreAssetInspector.paymentAvailability)
        {
          case AssetStoreAssetInspector.PaymentAvailability.BasketNotEmpty:
            ...
            break;
          case AssetStoreAssetInspector.PaymentAvailability.ServiceDisabled:
            if (firstAsset.previewInfo == null)
              break;
            AssetStore.Open(string.Format("content/{0}/directpurchase", (object) firstAsset.packageID));
            break;
          case AssetStoreAssetInspector.PaymentAvailability.AnonymousUser:
    ...
    {
      AssetStoreLoginWindow.Login("Please login to the Asset Store in order to get download information for the package.", (AssetStoreLoginWindow.LoginCallback) (errorMessage =>
      {
        if (errorMessage != null)
          return;
        AssetStoreAssetSelection.RefreshFromServer((AssetStoreAssetSelection.AssetsRefreshed) (() => AssetStoreInstaBuyWindow.ShowAssetStoreInstaBuyWindowBuilding(asset)));
      }));
    }
