...
namespace UnityEditor
{
  ...
  public sealed class HandleUtility
  {
    ...
    private static Vector3[] points = new Vector3[5]{ Vector3.zero, Vector3.zero, Vector3.zero, Vector3.zero, Vector3.zero };
    ...
    internal static float DistanceToDiamondInternal(Vector3 position, Quaternion rotation, float size, Vector2 mousePosition)
    {
      ...
      HandleUtility.points[0] = (Vector3) HandleUtility.WorldToGUIPoint(position + vector3_1);
      HandleUtility.points[1] = (Vector3) HandleUtility.WorldToGUIPoint(position - vector3_2);
      HandleUtility.points[2] = (Vector3) HandleUtility.WorldToGUIPoint(position - vector3_1);
      HandleUtility.points[3] = (Vector3) HandleUtility.WorldToGUIPoint(position + vector3_2);
      HandleUtility.points[4] = HandleUtility.points[0];
      ...
      for (int index2 = 0; index2 < 5; ++index2)
      {
        if ((double) HandleUtility.points[index2].y > (double) p.y != (double) HandleUtility.points[index1].y > (double) p.y && (double) p.x < ((double) HandleUtility.points[index1].x - (double) HandleUtility.points[index2].x) * ((double) p.y - (double) HandleUtility.points[index2].y) / ((double) HandleUtility.points[index1].y - (double) HandleUtility.points[index2].y) + (double) HandleUtility.points[index2].x)
          flag = !flag;
        index1 = index2;
      }
      if (flag)
      ...
      for (int index2 = 0; index2 < 4; ++index2)
      {
        float lineSegment = HandleUtility.DistancePointToLineSegment(p, (Vector2) HandleUtility.points[index2], (Vector2) HandleUtility.points[num2++]);
        if ((double) lineSegment < (double) num1 || (double) num1 < 0.0)
          num1 = lineSegment;
      }
      return num1;
    ...
    public static int nearestControl
    {
      get
      {
        return (double) HandleUtility.s_NearestDistance > 5.0 ? 0 : HandleUtility.s_NearestControl;
      }
      set
      {
    ...
    {
      Vector2 pixels = EditorGUIUtility.PointsToPixels(GUIClip.Unclip(guiPoint));
    ...
    public static Ray GUIPointToWorldRay(Vector2 position)
    {
      if ((bool) ((Object) Camera.current))
        return Camera.current.ScreenPointToRay((Vector3) HandleUtility.GUIPointToScreenPixelCoordinate(position));
    ...
    {
      Camera current = Camera.current;
      Vector2 pixels = EditorGUIUtility.PointsToPixels(guiPoint);
      pixels.y = current.pixelRect.yMax - pixels.y;
      return HandleUtility.Internal_FindNearestVertex(current, pixels, objectsToSearch, HandleUtility.ignoreRaySnapObjects, out vertex);
    }
    ...
    {
      return HandleUtility.PickGameObjectDelegated(position, (GameObject[]) null, (GameObject[]) null, out materialIndex);
    }

    public static GameObject PickGameObject(Vector2 position, GameObject[] ignore, out int materialIndex)
    {
      return HandleUtility.PickGameObjectDelegated(position, ignore, (GameObject[]) null, out materialIndex);
    }
    ...
    {
      int materialIndex;
      GameObject go = HandleUtility.PickGameObjectDelegated(position, ignore, filter, out materialIndex);
    ...
    private static Material handleWireMaterial
    {
      get
      {
        ...
        return !(bool) ((Object) Camera.current) ? HandleUtility.s_HandleWireMaterial2D : HandleUtility.s_HandleWireMaterial;
      }
    }
    ...
    private static Material handleDottedWireMaterial
    {
      ...
      {
        HandleUtility.InitHandleMaterials();
        return !(bool) ((Object) Camera.current) ? HandleUtility.s_HandleDottedWireMaterial2D : HandleUtility.s_HandleDottedWireMaterial;
      }
    }
    ...
    {
      HandleUtility.ApplyWireMaterial(CompareFunction.Always);
    }
    ...
    {
      HandleUtility.ApplyDottedWireMaterial(CompareFunction.Always);
    }
    ...
    {
      Material dottedWireMaterial = HandleUtility.handleDottedWireMaterial;
      dottedWireMaterial.SetInt("_HandleZTest", (int) zTest);
      dottedWireMaterial.SetPass(0);
