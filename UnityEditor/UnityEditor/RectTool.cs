  ...
  internal class RectTool : ManipulationTool
  {
    ...
    public override void ToolGUI(SceneView view, Vector3 handlePosition, bool isStatic)
    {
      ...
      if (!Tools.vertexDragging)
      {
        ...
        using (new EditorGUI.DisabledScope(!flag1 && !flag2))
        {
      ...
      EditorGUI.BeginChangeCheck();
      Vector3 position = RectTool.MoveHandlesGUI(handleRect, handlePosition, handleRectRotation);
      if (EditorGUI.EndChangeCheck() && !isStatic)
      {
        if (GridSnapping.active)
          position = GridSnapping.Snap(position);
        TransformManipulator.SetPositionDelta(position - TransformManipulator.mouseDownHandlePosition);
      }
      ...
      for (int xHandle = 0; xHandle <= 2; ++xHandle)
      {
        for (int yHandle = 0; yHandle <= 2; ++yHandle)
        {
          if (xHandle != 1 || yHandle != 1)
          {
            ...
            float size = 0.05f * HandleUtility.GetHandleSize(rectPointInWorld2);
            ...
            if ((double) GUI.color.a > 0.0 || GUIUtility.hotControl == controlId)
            {
              ...
              if (xHandle == 1 || yHandle == 1)
              {
                ...
                position = RectHandles.SideSlider(controlId, rectPointInWorld2, sideVector, direction, size, (Handles.CapFunction) null, 0.0f);
              }
              else
              {
                Vector3 vector3_2 = rotation * Vector3.right * (float) (xHandle - 1);
                Vector3 vector3_3 = rotation * Vector3.up * (float) (yHandle - 1);
                int id = controlId;
                Vector3 cornerPos = rectPointInWorld2;
                Vector3 handleDir = rotation * Vector3.forward;
                Vector3 outwardsDir1 = vector3_2;
                Vector3 outwardsDir2 = vector3_3;
                ...
                // ISSUE: reference to a compiler-generated field
                Handles.CapFunction fMgCache0 = RectTool.\u003C\u003Ef__mg\u0024cache0;
                Vector2 zero = Vector2.zero;
                position = RectHandles.CornerSlider(id, cornerPos, handleDir, outwardsDir1, outwardsDir2, (float) num, fMgCache0, zero);
              }
    ...
    private static Vector3 MoveHandlesGUI(Rect rect, Vector3 pivot, Quaternion rotation)
    {
      ...
      switch (typeForControl)
      {
        ...
        case EventType.Repaint:
          ...
          Handles.CircleHandleCap(controlId, pivot, rotation, num1, EventType.Repaint);
      ...
      while (xHandle <= 2)
      {
        ...
        while (yHandle <= 2)
        {
          ...
          if ((double) GUI.color.a > 0.0 || GUIUtility.hotControl == controlId)
          {
            ...
            float num = RectHandles.RotationSlider(controlId, rectPointInWorld, eulerAngles.z, pivot, rotation * Vector3.forward, outwardsDir1, outwardsDir2, handleSize, (Handles.CapFunction) null, Vector2.zero);
            if (EditorGUI.EndChangeCheck())
            {
    ...
    private static Vector3 PivotHandleGUI(Rect rect, Vector3 pivot, Quaternion rotation)
    {
      ...
      if ((double) GUI.color.a > 0.0 || GUIUtility.hotControl == controlId)
      {
        ...
        EditorGUI.BeginChangeCheck();
        int id = controlId;
        Vector3 handlePos = pivot;
        Vector3 handleDir = rotation * Vector3.forward;
        Vector3 slideDir1 = rotation * Vector3.right;
        Vector3 slideDir2 = rotation * Vector3.up;
        double num = (double) HandleUtility.GetHandleSize(pivot) * 0.100000001490116;
        // ISSUE: reference to a compiler-generated field
        ...
        // ISSUE: reference to a compiler-generated field
        Handles.CapFunction fMgCache1 = RectTool.\u003C\u003Ef__mg\u0024cache1;
        Vector2 zero = Vector2.zero;
        Vector3 vector3 = Handles.Slider2D(id, handlePos, handleDir, slideDir1, slideDir2, (float) num, fMgCache1, zero);
