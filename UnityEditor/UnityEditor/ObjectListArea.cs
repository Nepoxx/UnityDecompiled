...
namespace UnityEditor
{
  internal class ObjectListArea
  {
    ...
    internal Texture m_SelectedObjectIcon = (Texture) null;
    ...
    {
      return this.m_LocalAssets.GetVisibleNameAndInstanceIDs().Select<KeyValuePair<string, int>, string>((Func<KeyValuePair<string, int>, string>) (x => x.Key)).ToArray<string>();
    }
    ...
    public void Init(Rect rect, HierarchyType hierarchyType, SearchFilter searchFilter, bool checkThumbnails)
    {
      ...
      foreach (ObjectListArea.Group storeAsset in this.m_StoreAssets)
        storeAsset.UpdateFilter(hierarchyType, searchFilter, this.foldersFirst);
    ...
    private bool HasFocus()
    {
      if (!this.allowFocusRendering)
        return true;
      return this.m_KeyboardControlID == GUIUtility.keyboardControl && this.m_Owner.m_Parent.hasFocus;
    }
    ...
    private void QueryAssetStore()
    {
      ...
      ObjectListArea.\u003CQueryAssetStore\u003Ec__AnonStorey0 storeCAnonStorey0 = new ObjectListArea.\u003CQueryAssetStore\u003Ec__AnonStorey0();
      // ISSUE: reference to a compiler-generated field
      storeCAnonStorey0.\u0024this = this;
      ...
      else
      {
        ...
        // ISSUE: reference to a compiler-generated field
        storeCAnonStorey0.queryFilter = this.m_LastAssetStoreQuerySearchFilter + (object) this.m_LastAssetStoreQueryClassName + (object) this.m_LastAssetStoreQueryLabels;
        // ISSUE: reference to a compiler-generated method
        AssetStoreResultBase<AssetStoreSearchResults>.Callback callback = new AssetStoreResultBase<AssetStoreSearchResults>.Callback(storeCAnonStorey0.\u003C\u003Em__0);
        List<AssetStoreClient.SearchCount> counts = new List<AssetStoreClient.SearchCount>();
        if (!requeryAssetStore)
        {
          foreach (ObjectListArea.AssetStoreGroup storeAsset in this.m_StoreAssets)
          {
            AssetStoreClient.SearchCount searchCount = new AssetStoreClient.SearchCount();
            if (storeAsset.Visible && storeAsset.NeedItems)
            {
              searchCount.offset = storeAsset.Assets.Count;
              searchCount.limit = storeAsset.ItemsWantedShown - searchCount.offset;
            }
            searchCount.name = storeAsset.Name;
    ...
    private void EnsureAssetStoreGroupsAreOpenIfAllClosed()
    {
      ...
      if (num1 == 0)
      {
        foreach (ObjectListArea.AssetStoreGroup storeAsset in this.m_StoreAssets)
        {
          ObjectListArea.AssetStoreGroup assetStoreGroup = storeAsset;
          bool flag = true;
          storeAsset.visiblePreference = flag;
          int num2 = flag ? 1 : 0;
    ...
    private bool ObjectsHaveThumbnails(HierarchyType type, SearchFilter searchFilter)
    {
      if (this.m_LocalAssets.HasBuiltinResources)
        return true;
      IHierarchyProperty propertyForFilter = FilteredHierarchyProperty.CreateHierarchyPropertyForFilter(new FilteredHierarchy(type) { searchFilter = searchFilter });
    ...
    {
      return !this.m_LocalAssets.ListMode && !this.IsPreviewIconExpansionModifierPressed() && (!this.m_ShowLocalAssetsOnly || this.m_LocalAssets.ItemCount > 1);
    }
    ...
    private void SetSelectedAssetByIdx(int selectedIdx)
    {
      ...
      else
      {
        ...
        foreach (ObjectListArea.AssetStoreGroup storeAsset in this.m_StoreAssets)
        {
          if (!storeAsset.Visible)
          {
            height += storeAsset.Height;
          }
          else
          {
            AssetStoreAsset assetStoreResult = storeAsset.AssetAtIndex(selectedIdx);
            if (assetStoreResult != null)
            {
              this.ScrollToPosition(ObjectListArea.AdjustRectForFraming(storeAsset.m_Grid.CalcRect(selectedIdx, height)));
              this.Repaint();
              this.SetSelection(assetStoreResult, false);
              break;
            }
            selectedIdx -= storeAsset.m_Grid.rows * storeAsset.m_Grid.columns;
            height += storeAsset.Height;
          }
        }
      }
    ...
    private int GetSelectedAssetIdx()
    {
      ...
      foreach (ObjectListArea.AssetStoreGroup storeAsset in this.m_StoreAssets)
      {
        if (storeAsset.Visible)
        {
          int num3 = storeAsset.IndexOf(id);
          if (num3 != -1)
            return num2 + num3;
          num2 += storeAsset.m_Grid.rows * storeAsset.m_Grid.columns;
        }
      }
      return -1;
    }
    ...
    private void HandleListArea()
    {
      ...
      foreach (ObjectListArea.Group group in this.m_Groups)
      {
        if (!this.SkipGroup(group))
        {
          group.Draw(yOffset, scrollPosition, ref rowsInUse);
          flag2 = flag2 || group.NeedsRepaint;
          yOffset += group.Height;
          if (this.m_LocalAssets.ShowNone)
            break;
        }
      }
      ...
      else
      {
        ...
        if ((double) this.m_TotalRect.height < (double) num)
        {
          foreach (ObjectListArea.Group group in this.m_Groups)
          {
            if (!this.SkipGroup(group))
            {
              group.m_Grid.fixedWidth = this.m_TotalRect.width - 16f;
              group.m_Grid.InitNumRowsAndColumns(group.ItemCount, group.m_Grid.CalcRows(group.ItemsWantedShown));
              group.UpdateHeight();
              if (this.m_LocalAssets.ShowNone)
                break;
            }
      ...
      if (!this.m_InstanceIDToCroppedNameMap.TryGetValue(instanceID, out str))
      {
        ...
        switch (thatFitWithinWidth)
        {
          ...
          default:
            ...
            {
              str = fullText.Substring(0, thatFitWithinWidth - 1) + "…";
              break;
            }
            goto case 0;
        }
    ...
    public void BeginPing(int instanceID)
    {
      ...
      // ISSUE: object of a compiler-generated type is created
      // ISSUE: variable of a compiler-generated type
      ObjectListArea.\u003CBeginPing\u003Ec__AnonStorey4 pingCAnonStorey4 = new ObjectListArea.\u003CBeginPing\u003Ec__AnonStorey4();
      string fullText = (string) null;
      // ISSUE: reference to a compiler-generated field
      pingCAnonStorey4.hierarchyProperty = new HierarchyProperty(HierarchyType.Assets);
      // ISSUE: reference to a compiler-generated field
      if (pingCAnonStorey4.hierarchyProperty.Find(instanceID, (int[]) null))
      {
        // ISSUE: reference to a compiler-generated field
        fullText = pingCAnonStorey4.hierarchyProperty.name;
      }
      if (fullText == null)
        return;
      ...
      // ISSUE: reference to a compiler-generated field
      pingCAnonStorey4.label = content.text;
      if (this.m_LocalAssets.ListMode)
      {
        // ISSUE: object of a compiler-generated type is created
        // ISSUE: variable of a compiler-generated type
        ObjectListArea.\u003CBeginPing\u003Ec__AnonStorey3 pingCAnonStorey3 = new ObjectListArea.\u003CBeginPing\u003Ec__AnonStorey3();
        // ISSUE: reference to a compiler-generated field
        pingCAnonStorey3.\u003C\u003Ef__ref\u00244 = pingCAnonStorey4;
        ...
        // ISSUE: reference to a compiler-generated field
        this.m_LeftPaddingForPinging = !pingCAnonStorey4.hierarchyProperty.isMainRepresentation ? 28 : 13;
        // ISSUE: reference to a compiler-generated field
        pingCAnonStorey3.res = this.m_LocalAssets.LookupByInstanceID(instanceID);
        // ISSUE: reference to a compiler-generated method
        this.m_Ping.m_ContentDraw = new Action<Rect>(pingCAnonStorey3.\u003C\u003Em__0);
      }
      else
      {
        ...
        this.m_Ping.m_ContentRect.height = vector2.y;
        // ISSUE: reference to a compiler-generated method
        this.m_Ping.m_ContentDraw = new Action<Rect>(pingCAnonStorey4.\u003C\u003Em__0);
      }
    ...
    internal static event ObjectListArea.OnAssetIconDrawDelegate postAssetIconDrawCallback = null;

    internal static event ObjectListArea.OnAssetLabelDrawDelegate postAssetLabelDrawCallback = null;
    ...
    private class Styles
    {
      ...
      public GUIStyle groupFoldout = (GUIStyle) "IN Foldout";
    ...
    private abstract class Group
    {
      ...
      public void Draw(float yOffset, Vector2 scrollPos, ref int rowsInUse)
      {
        ...
        {
          int itemIdx = num1;
          int num2 = Math.Min(itemCount, this.m_Grid.rows * this.m_Grid.columns);
          int val1 = (int) Math.Ceiling((double) this.m_Owner.m_VisibleRect.height / (double) (this.m_Grid.itemSize.y + this.m_Grid.verticalSpacing)) + 1;
          int num3 = val1 - rowsInUse;
          if (num3 < 0)
            num3 = 0;
          rowsInUse = Math.Min(val1, Mathf.CeilToInt((float) (num2 - num1) / (float) this.m_Grid.columns));
          int endItem = num3 * this.m_Grid.columns + num1;
    ...
    private class LocalGroup : ObjectListArea.Group
    {
        ...
        switch (typeForControl)
        {
          case EventType.MouseDown:
            if (Event.current.button == 0 && rect.Contains(Event.current.mousePosition))
            {
              if (current.clickCount == 2)
              {
                this.m_Owner.SetSelection(new int[1]
                {
                  instanceID
                }, true);
            ...
            if (typeForControl != EventType.DragUpdated && typeForControl != EventType.DragPerform)
            {
              switch (typeForControl)
              {
                ...
                case EventType.ContextClick:
                  ...
                  if ((double) drawRect.width == (double) rect.width || !Provider.isActive || !drawRect.Contains(current.mousePosition))
                    return;
      ...
      private void SelectAndFrameParentOf(int instanceID)
      {
        ...
        if (instanceID1 == 0)
          return;
        this.m_Owner.SetSelection(new int[1]{ instanceID1 }, false);
      ...
      private void DrawItem(Rect position, FilteredHierarchy.FilterResult filterItem, BuiltinResource builtinResource, bool isFolderBrowsing)
      {
        ...
        Rect position1 = new Rect(position.x + (float) ObjectListArea.s_Styles.groupFoldout.margin.left, position.y, (float) ObjectListArea.s_Styles.groupFoldout.padding.left, position.height);
        ...
        if (flag4)
        {
          if (this.ListMode)
          {
            float num2 = (float) ((double) num3 + 16.0 + 2.0) + (float) ObjectListArea.s_Styles.resultsLabel.margin.left;
      ...
      private void RefreshBuiltinResourceList(SearchFilter searchFilter)
      {
        ...
        else
        {
          ...
          else
          {
            ...
            {
              UnityType nameCaseInsensitive = UnityType.FindTypeByNameCaseInsensitive(className);
              if (nameCaseInsensitive != null)
                intList.Add(nameCaseInsensitive.persistentTypeID);
            }
            if (intList.Count > 0)
            {
              foreach (KeyValuePair<string, BuiltinResource[]> builtinResource in this.m_BuiltinResourceMap)
              {
                UnityType typeByName = UnityType.FindTypeByName(builtinResource.Key);
                if (typeByName != null)
                {
                  foreach (int persistentTypeId in intList)
                  {
                    if (typeByName.IsDerivedFrom(UnityType.FindTypeByPersistentTypeID(persistentTypeId)))
                      builtinResourceList1.AddRange((IEnumerable<BuiltinResource>) builtinResource.Value);
                  }
                }
              }
            }
      ...
      {
        foreach (KeyValuePair<string, BuiltinResource[]> builtinResource1 in this.m_BuiltinResourceMap)
        {
          foreach (BuiltinResource builtinResource2 in builtinResource1.Value)
          {
            if (builtinResource2.m_InstanceID == instanceID)
              return true;
          }
        }
        return false;
      ...
      private void InitBuiltinAssetType(System.Type type)
      {
        ...
        else
        {
          ...
          UnityType typeByName = UnityType.FindTypeByName(str);
          if (typeByName == null)
          {
          ...
          {
            BuiltinResource[] builtinResourceList = EditorGUIUtility.GetBuiltinResourceList(typeByName.persistentTypeID);
      ...
      {
        string str = "" + "ObjectSelector -Builtin Assets Available:\n";
        foreach (KeyValuePair<string, BuiltinResource[]> builtinResource in this.m_BuiltinResourceMap)
        {
          BuiltinResource[] builtinResourceArray = builtinResource.Value;
          str = str + "    " + builtinResource.Key + ": ";
      ...
      public bool DoCharacterOffsetSelection()
      {
        if (Event.current.type == EventType.KeyDown && Event.current.shift)
        {
          ...
          string str2 = new string(new char[1]{ Event.current.character });
      ...
      public static void DrawIconAndLabel(Rect rect, FilteredHierarchy.FilterResult filterItem, string label, Texture2D icon, bool selected, bool focus)
      {
        ...
        float width = 28f;
        Rect drawRect = new Rect(rect.xMax - width, rect.y, width, rect.height);
        Rect position1 = new Rect(rect);
        if (ObjectListArea.LocalGroup.DrawExternalPostLabelInList(drawRect, filterItem))
          position1.width = rect.width - width;
        ObjectListArea.s_Styles.resultsLabel.padding.left = (int) ((double) num + 16.0 + 2.0);
        ObjectListArea.s_Styles.resultsLabel.Draw(position1, label, false, false, selected, focus);
        Rect position2 = rect;
        position2.width = 16f;
        position2.x += num * 0.5f;
        if ((UnityEngine.Object) icon != (UnityEngine.Object) null)
          GUI.DrawTexture(position2, (Texture) icon, ScaleMode.ScaleToFit);
        ...
        {
          // ISSUE: reference to a compiler-generated field
          ObjectListArea.postAssetIconDrawCallback(rect1, filterItem.guid, true);
        }
        ProjectHooks.OnProjectWindowItem(filterItem.guid, rect1);
      }
      ...
      private static bool DrawExternalPostLabelInList(Rect drawRect, FilteredHierarchy.FilterResult filterItem)
      {
        bool flag = false;
        // ISSUE: reference to a compiler-generated field
        if (filterItem != null && filterItem.guid != null && (filterItem.isMainRepresentation && ObjectListArea.postAssetLabelDrawCallback != null))
        {
          // ISSUE: reference to a compiler-generated field
          flag = ObjectListArea.postAssetLabelDrawCallback(drawRect, filterItem.guid, true);
        }
        return flag;
      }
