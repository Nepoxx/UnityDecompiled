...
namespace UnityEditor
{
  ...
  {
    private static readonly ColorPickerHDRConfig m_DefaultHDRConfig = new ColorPickerHDRConfig(0.0f, 65536f, 1.525879E-05f, 3f);
    ...
    private string[] m_ColorBoxXAxisLabels = new string[7]{ "Saturation", "Hue", "Hue", "Blue", "Blue", "Red", "" };
    private string[] m_ColorBoxYAxisLabels = new string[7]{ "Brightness", "Brightness", "Saturation", "Green", "Red", "Green", "" };
    private string[] m_ColorBoxZAxisLabels = new string[7]{ "Hue", "Saturation", "Brightness", "Red", "Green", "Blue", "" };
    [SerializeField]
    ...
    public static ColorPicker get
    {
      get
      {
        ...
        {
          UnityEngine.Object[] objectsOfTypeAll = UnityEngine.Resources.FindObjectsOfTypeAll(typeof (ColorPicker));
    ...
    private void RGBSliders()
    {
      ...
      this.m_R = this.TexturedSlider(this.m_RTexture, "R", this.m_R, 0.0f, 1f, displayScale, formatString, new Action<float>(this.OnFloatFieldChanged));
      this.m_G = this.TexturedSlider(this.m_GTexture, "G", this.m_G, 0.0f, 1f, displayScale, formatString, new Action<float>(this.OnFloatFieldChanged));
      this.m_B = this.TexturedSlider(this.m_BTexture, "B", this.m_B, 0.0f, 1f, displayScale, formatString, new Action<float>(this.OnFloatFieldChanged));
    ...
    private float TexturedSlider(Texture2D background, string text, float val, float min, float max, float displayScale, string formatString, Action<float> onFloatFieldChanged)
    {
      ...
      int controlId = GUIUtility.GetControlID(869045, FocusType.Keyboard, this.position);
      EditorGUI.BeginChangeCheck();
    ...
    private void HSVSliders()
    {
      ...
      float displayScale = !this.m_HDR ? (float) byte.MaxValue : colorScale;
      string formatString = !this.m_HDR ? EditorGUI.kIntFieldFormatString : EditorGUI.kFloatFieldFormatString;
      this.m_H = this.TexturedSlider(this.m_HueTexture, "H", this.m_H, 0.0f, 1f, 359f, EditorGUI.kIntFieldFormatString, (Action<float>) null);
      this.m_S = this.TexturedSlider(this.m_SatTexture, "S", this.m_S, 0.0f, 1f, !this.m_HDR ? (float) byte.MaxValue : 1f, formatString, (Action<float>) null);
      this.m_V = this.TexturedSlider(this.m_ValTexture, "V", this.m_V, 0.0f, 1f, displayScale, formatString, (Action<float>) null);
      if (!EditorGUI.EndChangeCheck())
        return;
      this.HSVToRGB();
    }
    ...
    private void InitIfNeeded()
    {
      ...
      if (this.m_ColorLibraryEditor != null)
        return;
      this.m_ColorLibraryEditor = new PresetLibraryEditor<ColorPresetLibrary>(new ScriptableObjectSaveLoadHelper<ColorPresetLibrary>("colors", SaveType.Text), this.m_ColorLibraryEditorState, new Action<int, object>(this.PresetClickedCallback));
    ...
    private void DoColorSliders()
    {
      ...
      this.m_A = this.TexturedSlider(this.m_AlphaTexture, "A", this.m_A, 0.0f, 1f, !this.m_HDR ? (float) byte.MaxValue : 1f, !this.m_HDR ? EditorGUI.kIntFieldFormatString : EditorGUI.kFloatFieldFormatString, (Action<float>) null);
    }
    ...
    private void TonemappingControls()
    {
      ...
      if (this.m_UseTonemappingPreview)
      {
        ++EditorGUI.indentLevel;
        EditorGUI.BeginChangeCheck();
        this.m_HDRValues.m_ExposureAdjustment = EditorGUILayout.PowerSlider("", this.m_HDRValues.m_ExposureAdjustment, this.m_HDRConfig.minExposureValue, this.m_HDRConfig.maxExposureValue, QualitySettings.activeColorSpace != ColorSpace.Linear ? 2f : 1f);
        if (Event.current.type == EventType.Repaint)
    ...
    public static void Show(Action<Color> onColorChanged, Color col, bool showAlpha, bool hdr, ColorPickerHDRConfig hdrConfig)
    {
      ...
      colorPicker.m_DelegateView = (GUIView) null;
    ...
    public static void Show(GUIView viewToUpdate, Color col, bool showAlpha, bool hdr, ColorPickerHDRConfig hdrConfig)
    {
      ...
      colorPicker.m_OnColorChanged = (Action<Color>) null;
    }
