  ...
  internal class CubemapInspector : TextureInspector
  {
    ...
    private void InitTexturesFromCubemap()
    {
      ...
      for (int index = 0; index < this.m_Images.Length; ++index)
      {
        ...
        {
          this.m_Images[index] = new Texture2D(64, 64, TextureFormat.RGBA32, false);
    ...
    public override void OnInspectorGUI()
    {
      ...
      int num = EditorGUILayout.IntPopup("Face size", TextureUtil.GetGPUWidth((Texture) target), CubemapInspector.kSizes, CubemapInspector.kSizesValues, new GUILayoutOption[0]);
      bool useMipmap = EditorGUILayout.Toggle("MipMaps", TextureUtil.GetMipmapCount((Texture) target) > 1, new GUILayoutOption[0]);
