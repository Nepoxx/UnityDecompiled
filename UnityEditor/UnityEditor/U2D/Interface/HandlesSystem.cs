...
// Type: UnityEditor.U2D.Interface.HandlesSystem
  ...
  internal class HandlesSystem : IHandles
  {
    ...
    public static IHandles GetSystem()
    {
      if (HandlesSystem.m_System == null)
        HandlesSystem.m_System = (IHandles) new HandlesSystem();
    ...
    {
      Handles.DrawAAPolyLine((UnityEngine.Texture2D) lineTex, width, points);
    }

    public void DrawAAPolyLine(ITexture2D lineTex, params Vector3[] points)
    {
      Handles.DrawAAPolyLine((UnityEngine.Texture2D) lineTex, points);
    }
