...
namespace UnityEditor
{
  ...
  public sealed class BuildPipeline
  {
    ...
    {
      return BuildPipeline.BuildPlayer(new BuildPlayerOptions() { scenes = EditorBuildSettingsScene.GetActiveSceneList(levels), locationPathName = locationPathName, target = target, options = options });
    }
    ...
    public static string BuildPlayer(string[] levels, string locationPathName, BuildTarget target, BuildOptions options)
    {
      ...
      return BuildPipeline.BuildPlayer(new BuildPlayerOptions() { scenes = levels, locationPathName = locationPathName, targetGroup = buildTargetGroup, target = target, options = options });
    }
    ...
    internal static string[] GetReferencingPlayerAssembliesForDLL(string dllPath)
    {
      ...
      AssemblyDefinition assemblyDefinition = AssemblyDefinition.ReadAssembly(dllPath, new ReaderParameters() { AssemblyResolver = (IAssemblyResolver) assemblyResolver1 });
      ...
      foreach (string path in managedPlayerDllPaths)
      {
        ...
        ReaderParameters parameters = new ReaderParameters() { AssemblyResolver = (IAssemblyResolver) assemblyResolver2 };
