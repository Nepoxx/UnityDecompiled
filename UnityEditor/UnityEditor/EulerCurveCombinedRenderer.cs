...
namespace UnityEditor
{
  internal class EulerCurveCombinedRenderer
  {
    ...
    private float rangeStart
    {
      get
      {
        return (double) this.m_CustomRangeStart != 0.0 || (double) this.m_CustomRangeEnd != 0.0 || this.eulerX.length <= 0 ? this.m_CustomRangeStart : this.eulerX.keys[0].time;
      }
    }
    ...
    private float rangeEnd
    {
      get
      {
        return (double) this.m_CustomRangeStart != 0.0 || (double) this.m_CustomRangeEnd != 0.0 || this.eulerX.length <= 0 ? this.m_CustomRangeEnd : this.eulerX.keys[this.eulerX.length - 1].time;
      }
    }
    ...
    public float EvaluateCurveSlow(float time, int component)
    {
      ...
      int index1 = 0;
      foreach (KeyValuePair<float, Vector3> point in this.points)
      {
        numArray[index1] = point.Key;
        vector3Array[index1] = point.Value;
        ++index1;
      }
    ...
    public void DrawCurve(float minTime, float maxTime, Color color, Matrix4x4 transform, int component, Color wrapColor)
    {
      ...
      foreach (KeyValuePair<float, Vector3> point in this.points)
        vector3List.Add(new Vector3(point.Key, point.Value[component]));
    ...
    public Bounds GetBounds(float minTime, float maxTime, int component)
    {
      ...
      foreach (KeyValuePair<float, Vector3> point in this.points)
      {
        if ((double) point.Value[component] > (double) num2)
          num2 = point.Value[component];
        if ((double) point.Value[component] < (double) num1)
          num1 = point.Value[component];
      }
