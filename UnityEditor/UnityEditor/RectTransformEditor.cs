  ...
  internal class RectTransformEditor : Editor
  {
    ...
    private void HandleDragChange(string handleName, bool dragging)
    {
      ...
      if (handleName != null)
      {
        ...
        if (RectTransformEditor.\u003C\u003Ef__switch\u0024map1 == null)
        {
          // ISSUE: reference to a compiler-generated field
          RectTransformEditor.\u003C\u003Ef__switch\u0024map1 = new Dictionary<string, int>(9)
          {
            {
              "ChangingLeft",
              0
        ...
        int num;
        // ISSUE: reference to a compiler-generated field
        if (RectTransformEditor.\u003C\u003Ef__switch\u0024map1.TryGetValue(handleName, out num))
        {
          switch (num)
          {
            case 0:
...
label_16:
    ...
    private static bool ShouldDoIntSnapping(RectTransform rect)
    {
      ...
      return (UnityEngine.Object) componentInParent != (UnityEngine.Object) null && componentInParent.renderMode != UnityEngine.RenderMode.WorldSpace;
    }
    ...
    private static void Vector3FieldWithDisabledMash(Rect position, SerializedProperty property, GUIContent label, bool[] disabledMask)
    {
      ...
      EditorGUI.MultiPropertyField(position, RectTransformEditor.s_XYZLabels, valuesIterator, EditorGUI.PropertyVisibility.OnlyVisible, 13f, disabledMask);
    ...
    private void LayoutDropdownButton(bool anyWithoutParent)
    {
      ...
      using (new EditorGUI.DisabledScope(anyWithoutParent))
      {
        ...
        if (EditorGUI.DropdownButton(rect, GUIContent.none, FocusType.Passive, (GUIStyle) "box"))
        {
          GUIUtility.keyboardControl = 0;
          this.m_DropdownWindow = new LayoutDropdownWindow(this.serializedObject);
          PopupWindow.Show(rect, (PopupWindowContent) this.m_DropdownWindow, (PopupLocationHelper.PopupLocation[]) null, ShowMode.PopupMenuWithKeyboardFocus);
        }
        GUI.color = color;
      }
      if (anyWithoutParent)
    ...
    {
      using (new EditorGUI.DisabledScope(((IEnumerable<UnityEngine.Object>) this.targets).Any<UnityEngine.Object>((Func<UnityEngine.Object, bool>) (x => ((x as RectTransform).drivenProperties & driven) != DrivenTransformProperties.None))))
      {
        float num = getter(this.target as RectTransform);
        EditorGUI.showMixedValue = ((IEnumerable<UnityEngine.Object>) this.targets).Select<UnityEngine.Object, float>((Func<UnityEngine.Object, float>) (x => getter(x as RectTransform))).Distinct<float>().Count<float>() >= 2;
        EditorGUI.BeginChangeCheck();
    ...
    private void Vector2Field(Rect position, RectTransformEditor.FloatGetter xGetter, RectTransformEditor.FloatSetter xSetter, RectTransformEditor.FloatGetter yGetter, RectTransformEditor.FloatSetter ySetter, DrivenTransformProperties xDriven, DrivenTransformProperties yDriven, SerializedProperty vec2Property, GUIContent label)
    {
      ...
      SerializedProperty propertyRelative1 = vec2Property.FindPropertyRelative("x");
      SerializedProperty propertyRelative2 = vec2Property.FindPropertyRelative("y");
      ...
      EditorGUI.BeginProperty(columnRect1, RectTransformEditor.s_XYLabels[0], propertyRelative1);
      this.FloatField(columnRect1, xGetter, xSetter, xDriven, RectTransformEditor.s_XYLabels[0]);
      EditorGUI.EndProperty();
      EditorGUI.BeginProperty(columnRect1, RectTransformEditor.s_XYLabels[1], propertyRelative2);
    ...
    private void ParentRectPreviewDragHandles(RectTransform gui, Transform space)
    {
      ...
      for (int index1 = 0; index1 <= 2; ++index1)
      {
        for (int index2 = 0; index2 <= 2; ++index2)
        {
          ...
          {
            Vector3 position1 = (Vector3) Vector2.zero;
            for (int index3 = 0; index3 < 2; ++index3)
              position1[index3] = Mathf.Lerp(rect1.min[index3], rect1.max[index3], (float) ((index3 != 0 ? (double) index2 : (double) index1) * 0.5));
            position1 = space.TransformPoint(position1);
            ...
            {
              EditorGUI.BeginChangeCheck();
              Vector3 position2 = RectHandles.SideSlider(controlId, position1, sideVector, direction, size, (Handles.CapFunction) null, 0.0f, -3f);
              if (EditorGUI.EndChangeCheck())
              {
    ...
    private static bool AnchorAllowedOutsideParent(int axis, int minmax)
    {
      ...
      return (double) num < -1.0 / 1000.0 || (double) num > 1.00100004673004;
    }
    ...
    private void AnchorSceneGUI(RectTransform gui, RectTransform guiParent, Transform parentSpace, bool interactive, int minmaxX, int minmaxY, int id)
    {
      ...
      EditorGUI.BeginChangeCheck();
      Vector3 position = Handles.Slider2D(id, vector3, parentSpace.forward, parentSpace.right, parentSpace.up, handleSize, (Handles.CapFunction) null, Vector2.zero);
        ...
        for (int index2 = 0; index2 < 4; ++index2)
        {
          ...
          switch (index2)
          {
            case 0:
              ...
              break;
            case 1:
              vector3 = (Vector3) gui.anchorMin;
              break;
            case 2:
              vector3 = (Vector3) gui.anchorMax;
              break;
            case 3:
              vector3 = Vector3.one;
              break;
          }
          vector3[index1] = gui.anchorMin[index1];
          vector3 = parentSpace.TransformPoint(this.GetAnchorLocal(guiParent, (Vector2) vector3));
          vector3Array[index1, index2] = vector3;
        }
      }
    ...
    public static void SetAnchorSmart(RectTransform rect, float value, int axis, bool isMax, bool smart, bool enforceExactValue, bool enforceMinNoLargerThanMax, bool moveTogether)
    {
      ...
      if (!moveTogether)
      {
        ...
        // ISSUE: explicit reference operation
        // ISSUE: explicit reference operation
        double num3 = (double) (^(local = @sizeDelta))[index = axis] + (double) num1 * (!isMax ? 1.0 : -1.0);
        // ISSUE: explicit reference operation
        (^local)[index] = (float) num3;
