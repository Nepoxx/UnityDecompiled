...
namespace UnityEditor
{
  ...
  internal class SpriteInspector : Editor
  {
    ...
    public override void OnInspectorGUI()
    {
      ...
      bool alignment1;
      bool border1;
      this.UnifiedValues(out name, out alignment1, out border1);
      if (name)
      ...
      if (alignment1)
      {
        int alignment2 = this.GetMetaData(this.sprite.name).alignment;
        EditorGUILayout.LabelField(SpriteInspector.Styles.spriteAlignment, SpriteInspector.Styles.spriteAlignmentOptions[alignment2], new GUILayoutOption[0]);
      }
      else
      ...
      for (int index1 = 0; index1 < this.targets.Length; ++index1)
      {
        ...
        for (int index2 = 0; index2 < spritesheet.Length; ++index2)
        {
          if (spritesheet[index2].name.Equals(target.name))
          {
            ...
            Vector4 border1 = spritesheet[index2].border;
            if ((!nullable.HasValue ? 1 : (border1 != nullable.GetValueOrDefault() ? 1 : 0)) != 0 && nullable.HasValue)
              border = false;
            else
    ...
    public static Texture2D BuildPreviewTexture(int width, int height, Sprite sprite, Material spriteRendererMaterial, bool isPolygon)
    {
      ...
      if ((Object) spriteRendererMaterial != (Object) null)
      {
        ...
        if (flag2)
        {
          vector4 = spriteRendererMaterial.GetVector("_MainTex_TexelSize");
      ...
      if ((Object) spriteRendererMaterial != (Object) null)
      {
        ...
        if (flag2)
          spriteRendererMaterial.SetVector("_MainTex_TexelSize", vector4);
      }
