...
{
  [EditorWindowTitle(icon = "UnityEditor.VersionControl", title = "Version Control")]
  internal class WindowPending : EditorWindow
  {
    private static WindowPending.Styles s_Styles = (WindowPending.Styles) null;
    private static Texture2D changeIcon = (Texture2D) null;
    private static bool s_DidReload = false;
    private Texture2D syncIcon = (Texture2D) null;
    private Texture2D refreshIcon = (Texture2D) null;
    private bool m_ShowIncoming = false;
    private float s_ToolbarButtonsWidth = 0.0f;
    private float s_SettingsButtonWidth = 0.0f;
    private float s_DeleteChangesetsButtonWidth = 0.0f;
    ...
    {
      foreach (WindowPending windowPending in UnityEngine.Resources.FindObjectsOfTypeAll(typeof (WindowPending)) as WindowPending[])
    ...
    {
      WindowPending[] objectsOfTypeAll = UnityEngine.Resources.FindObjectsOfTypeAll(typeof (WindowPending)) as WindowPending[];
    ...
    private void PopulateListControl(ListControl list, Task task, Texture2D icon)
    {
      ...
      while (listItem1 != null)
      {
        ...
        if (changeSets.Find(new Predicate<ChangeSet>(new WindowPending.\u003CPopulateListControl\u003Ec__AnonStorey0() { cs = listItem1.Item as ChangeSet }.\u003C\u003Em__0)) == null)
        {
      ...
      foreach (ChangeSet change in (List<ChangeSet>) changeSets)
      {
        ListItem listItem2 = list.GetChangeSetItem(change);
        if (listItem2 != null)
        {
          listItem2.Item = (object) change;
          listItem2.Name = change.description;
        }
        else
          listItem2 = list.Add((ListItem) null, change.description, change);
    ...
    private void OnChangeContents(Task task)
    {
      ...
      {
        foreach (Asset asset in (List<Asset>) assetList)
          listControl.Add(parent, asset.prettyPath, asset);
      }
    ...
    private ChangeSets GetEmptyChangeSetsCandidates()
    {
      ...
      WindowPending.\u003CGetEmptyChangeSetsCandidates\u003Ec__AnonStorey1 candidatesCAnonStorey1 = new WindowPending.\u003CGetEmptyChangeSetsCandidates\u003Ec__AnonStorey1();
      ChangeSets emptyChangeSets = this.pendingList.EmptyChangeSets;
      // ISSUE: reference to a compiler-generated field
      candidatesCAnonStorey1.toDelete = new ChangeSets();
      // ISSUE: reference to a compiler-generated method
      emptyChangeSets.FindAll((Predicate<ChangeSet>) (item => item.id != ChangeSet.defaultID)).ForEach(new Action<ChangeSet>(candidatesCAnonStorey1.\u003C\u003Em__0));
      // ISSUE: reference to a compiler-generated field
      return candidatesCAnonStorey1.toDelete;
    }
    ...
    private void OnGUI()
    {
      ...
      this.m_ShowIncoming = GUILayout.Toggle(this.m_ShowIncoming, GUIContent.Temp("Incoming" + (num != 0 ? " (" + num.ToString() + ")" : "")), EditorStyles.toolbarButton, new GUILayoutOption[0]);
