...
namespace UnityEditor
{
  internal class SerializedMinMaxCurve
  {
    ...
    public bool OnCurveAreaMouseDown(int button, Rect drawRect, Rect curveRanges)
    {
      switch (button)
      {
        ...
        case 1:
          ...
          AnimationCurveContextMenu.Show(drawRect, this.maxCurve == null ? (SerializedProperty) null : this.maxCurve.Copy(), minCurve == null ? (SerializedProperty) null : minCurve.Copy(), this.scalar == null ? (SerializedProperty) null : this.scalar.Copy(), curveRanges, this.m_Module.GetParticleSystemCurveEditor());
          return true;
        default:
          return false;
      }
    ...
    {
      return this.state != MinMaxCurveState.k_TwoCurves ? (SerializedProperty) null : this.minCurve;
    }
    ...
    private static bool AnimationCurveSupportsProcedural(AnimationCurve curve, ref string failureReason)
    {
      switch (AnimationUtility.IsValidPolynomialCurve(curve))
      {
        ...
        case AnimationUtility.PolynomialValid.TooManySegments:
          failureReason = "Curve uses too many keys. Procedural mode does not support more than " + (object) AnimationUtility.GetMaxNumPolynomialSegmentsSupported() + " keys";
          if ((double) curve.keys[0].time != 0.0 || (double) curve.keys[curve.keys.Length - 1].time != 1.0)
    ...
    {
      string failureReason1 = "Max Curve: ";
      bool flag1 = SerializedMinMaxCurve.AnimationCurveSupportsProcedural(this.maxCurve.animationCurveValue, ref failureReason1);
      if (!flag1)
        failureReason = failureReason1;
      if (this.state != MinMaxCurveState.k_TwoCurves && this.state != MinMaxCurveState.k_TwoScalars)
        return flag1;
      string failureReason2 = "Min Curve: ";
      bool flag2 = SerializedMinMaxCurve.AnimationCurveSupportsProcedural(this.minCurve.animationCurveValue, ref failureReason2);
      if (flag2)
