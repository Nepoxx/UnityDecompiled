...
namespace UnityEditor
{
  internal class ModelImporterClipEditor : BaseAssetImporterTabUI
  {
    ...
    private SerializedObject m_DefaultClipsSerializedObject = (SerializedObject) null;
    private AvatarMask m_Mask = (AvatarMask) null;
    private AvatarMaskInspector m_MaskInspector = (AvatarMaskInspector) null;
    ...
    internal override void OnEnable()
    {
      ...
      this.m_ResampleCurves = this.serializedObject.FindProperty("m_ResampleCurves");
      ...
      this.m_ImportAnimatedCustomProperties = this.serializedObject.FindProperty("m_ImportAnimatedCustomProperties");
    ...
    public override void OnInspectorGUI()
    {
      ...
      if (!this.m_ImportAnimation.boolValue || this.m_ImportAnimation.hasMultipleDifferentValues)
        return;
      bool flag = this.targets.Length == 1 && this.singleImporter.importedTakeInfos.Length == 0 && this.singleImporter.animationType != ModelImporterAnimationType.None;
    ...
    private void RootMotionNodeSettings()
    {
      ...
      if (ModelImporterClipEditor.motionNodeFoldout)
      {
        ...
        if (EditorGUI.EndChangeCheck())
          this.m_MotionNodeName.stringValue = this.motionNodeIndex <= 0 || this.motionNodeIndex >= this.m_MotionNodeList.Length ? "" : this.m_MotionNodeList[this.motionNodeIndex].text;
      }
    }
    ...
    private void DestroyEditorsAndData()
    {
      ...
      {
        this.DestroyImmediate((UnityEngine.Object) this.m_MaskInspector);
        this.m_MaskInspector = (AvatarMaskInspector) null;
      }
      if (!(bool) ((UnityEngine.Object) this.m_Mask))
        return;
      this.DestroyImmediate((UnityEngine.Object) this.m_Mask);
    ...
    {
      return this.m_ImportAnimation.boolValue && (UnityEngine.Object) this.m_AnimationClipEditor != (UnityEngine.Object) null && this.m_AnimationClipEditor.HasPreviewGUI();
    }
    ...
    {
      if (this.animationType == ModelImporterAnimationType.Legacy)
        return this.legacyGenerateAnimations == ModelImporterGenerateAnimations.InOriginalRoots || this.legacyGenerateAnimations == ModelImporterGenerateAnimations.InNodes;
      return false;
    }
    ...
    private string RemoveDuplicateSuffix(string name, out int number)
    {
      ...
      if (length1 < 4 || (int) name[length1 - 1] != 41)
        return name;
      int num1 = name.LastIndexOf('(', length1 - 2);
      if (num1 == -1 || (int) name[num1 - 1] != 32)
        return name;
      int length2 = length1 - num1 - 2;
      if (length2 == 0)
        return name;
      int num2 = 0;
      while (num2 < length2 && char.IsDigit(name[num1 + 1 + num2]))
        ++num2;
      if (num2 != length2)
        return name;
      string s = name.Substring(num1 + 1, length2);
      number = int.Parse(s);
      return name.Substring(0, num1 - 1);
    }
    ...
    private int FindNextDuplicateNumber(string baseName)
    {
      ...
      {
        int number;
        if (this.RemoveDuplicateSuffix(this.GetAnimationClipInfoAtIndex(index).name, out number) == baseName)
        {
          if (val1 == -1)
            val1 = 1;
          if (number != -1)
            val1 = Math.Max(val1, number + 1);
        }
      }
      return val1;
    }
    ...
    {
      int number;
      string baseName = this.RemoveDuplicateSuffix(name, out number);
      int nextDuplicateNumber = this.FindNextDuplicateNumber(baseName);
      if (nextDuplicateNumber != -1)
        name = baseName + " (" + (object) nextDuplicateNumber + ")";
      return name;
    }
    ...
    {
      string str = this.MakeUniqueClipName(takeInfo.defaultClipName);
    ...
    private void SetTransformMaskFromReference(AnimationClipInfoProperties clipInfo)
    {
      ...
      string[] currentPaths = this.animationType != ModelImporterAnimationType.Human ? AvatarMaskUtility.GetAvatarInactiveTransformMaskPaths(clipInfo.transformMaskProperty) : AvatarMaskUtility.GetAvatarHumanAndActiveExtraTransforms(this.serializedObject, clipInfo.transformMaskProperty, referenceTransformPaths);
