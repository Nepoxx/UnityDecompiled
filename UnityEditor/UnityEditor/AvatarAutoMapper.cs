...
namespace UnityEditor
{
  internal class AvatarAutoMapper
  {
    ...
    public Dictionary<int, Transform> MapBones(Transform root)
    {
      ...
      if (!mapping.ContainsKey(8) && mapping.ContainsKey(54))
      {
        mapping.Add(8, mapping[54]);
        mapping.Remove(54);
      }
      int num = 3;
    ...
    private void ApplyMapping(AvatarAutoMapper.BoneMatch match, Dictionary<int, Transform> mapping)
    {
      ...
      foreach (AvatarAutoMapper.BoneMatch child in match.children)
        this.ApplyMapping(child, mapping);
    }
    ...
    {
      return boneName.ToLower().Contains(!left ? "right" : nameof (left)) || Regex.Match(boneName, !left ? "(^|.*[ \\.:_-])[rR]($|[ \\.:_-].*)" : "(^|.*[ \\.:_-])[lL]($|[ \\.:_-].*)").Length > 0;
    }
            ...
            while (enumerator.MoveNext())
            {
              ...
              if (this.m_ValidBones == null || this.m_ValidBones.ContainsKey(current))
              {
                if (!this.m_TreatDummyBonesAsReal && this.m_ValidBones != null && !this.m_ValidBones[current])
                  queuedBoneQueue.Enqueue(new AvatarAutoMapper.QueuedBone(current, queuedBone.level));
                else
                  queuedBoneQueue.Enqueue(new AvatarAutoMapper.QueuedBone(current, queuedBone.level + 1));
              }
            }
          }
          finally
    ...
    private string GetNameOfBone(int boneIndex)
    {
      ...
      return "" + (object) (HumanBodyBones) boneIndex;
    }
    ...
    {
      string str1 = this.GetNameOfBone(matches[0].item.bone) + " preferred order: ";
      for (int index = 0; index < matches.Count; ++index)
        str1 = str1 + matches[index].bone.name + " (" + matches[index].score.ToString("0.0") + " / " + matches[index].totalSiblingScore.ToString("0.0") + "), ";
      foreach (AvatarAutoMapper.BoneMatch match in matches)
      {
    ...
    private int[] GetBestChildMatchChoices(AvatarAutoMapper.BoneMatch parentMatch, List<List<AvatarAutoMapper.BoneMatch>> childMatchesLists, int[] choices, out float score)
    {
      ...
      if (intList.Count <= 1)
      {
        ...
        {
          if (choices[index] >= 0)
            score += childMatchesLists[index][choices[index]].totalSiblingScore;
        }
        return choices;
      }
      float num = 0.0f;
    ...
    private void EvaluateBoneMatch(AvatarAutoMapper.BoneMatch match, bool confirmedChoice)
    {
      ...
      if (childMatchesLists.Count > 0)
      {
        ...
        {
          if (AvatarAutoMapper.kDebug && confirmedChoice)
            this.EvaluateBoneMatch(child, confirmedChoice);
          ++num2;
          match.score += child.score;
          if (AvatarAutoMapper.kDebug)
            match.debugTracker.AddRange((IEnumerable<string>) child.debugTracker);
          if ((UnityEngine.Object) child.bone == (UnityEngine.Object) match.bone && child.item.bone >= 0)
            flag = true;
        }
      }
