...
namespace UnityEditor
{
  internal static class LODGroupGUI
  {
    ...
    {
      return LODGroupGUI.LinearizeScreenPercentage(Mathf.Clamp((float) (1.0 - ((double) position.x - (double) sliderRect.x) / (double) sliderRect.width), 0.01f, 1f));
    }

    public static void SetSelectedLODLevelPercentage(float newScreenPercentage, int lod, List<LODGroupGUI.LODInfo> lods)
    {
      float num1 = 0.0f;
      LODGroupGUI.LODInfo lodInfo1 = lods.FirstOrDefault<LODGroupGUI.LODInfo>((Func<LODGroupGUI.LODInfo, bool>) (x => x.LODLevel == lods[lod].LODLevel + 1));
      if (lodInfo1 != null)
        num1 = lodInfo1.RawScreenPercent;
      float num2 = 1f;
      LODGroupGUI.LODInfo lodInfo2 = lods.FirstOrDefault<LODGroupGUI.LODInfo>((Func<LODGroupGUI.LODInfo, bool>) (x => x.LODLevel == lods[lod].LODLevel - 1));
      if (lodInfo2 != null)
        num2 = lodInfo2.RawScreenPercent;
      float max = Mathf.Clamp01(num2);
      float min = Mathf.Clamp01(num1);
      lods[lod].RawScreenPercent = Mathf.Clamp(newScreenPercentage, min, max);
    }
