...
namespace UnityEditor
{
  ...
  {
    private WebTemplate[] s_Templates = (WebTemplate[]) null;
    private GUIContent[] s_TemplateGUIThumbnails = (GUIContent[]) null;
    ...
    private WebTemplate Load(string path)
    {
      ...
      IEnumerator enumerator = new Regex("\\%UNITY_CUSTOM_([A-Z_]+)\\%").Matches(File.ReadAllText(Directory.GetFiles(path, "index.*")[0])).GetEnumerator();
      try
      {
        while (enumerator.MoveNext())
        {
          string str1 = ((Capture) enumerator.Current).Value.Substring("%UNITY_CUSTOM_".Length);
