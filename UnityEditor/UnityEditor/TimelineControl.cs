...
// Type: UnityEditor.TimelineControl
...
namespace UnityEditor
{
  internal class TimelineControl
  {
    ...
    private float m_SrcStartTime = 0.0f;
    ...
    private float m_DstDragOffset = 0.0f;
    private float m_LeftThumbOffset = 0.0f;
    private float m_RightThumbOffset = 0.0f;
    private float m_TimeStartDrag = 0.0f;
    private float m_TimeOffset = 0.0f;
    ...
    private Vector3[] GetPivotVectors(TimelineControl.PivotSample[] samples, float width, Rect rect, float height, bool loop)
    {
      ...
      {
        TimelineControl.PivotSample sample = samples[index];
    ...
    {
      return (double) this.m_DstDragOffset != 0.0 || (double) this.m_LeftThumbOffset != 0.0 || (double) this.m_RightThumbOffset != 0.0;
    }
    ...
    public bool DoTimeline(Rect timeRect)
    {
      ...
      if (current.type == EventType.MouseDown && rect1.Contains(current.mousePosition))
      {
        ...
        if (position7.Contains(current.mousePosition))
        {
          ...
          this.m_TimeStartDrag = this.m_TimeArea.TimeToPixel(this.Time, rect1);
        }
        else
          this.m_DragState = !rect2.Contains(current.mousePosition) ? (!rect3.Contains(current.mousePosition) ? (!position5.Contains(current.mousePosition) ? (!position6.Contains(current.mousePosition) ? (!position3.Contains(current.mousePosition) ? (!position1.Contains(current.mousePosition) ? (!position2.Contains(current.mousePosition) ? TimelineControl.DragStates.None : TimelineControl.DragStates.TimeArea) : TimelineControl.DragStates.TimeArea) : TimelineControl.DragStates.FullSelection) : TimelineControl.DragStates.RightSelection) : TimelineControl.DragStates.LeftSelection) : TimelineControl.DragStates.Destination) : TimelineControl.DragStates.Source;
        current.Use();
      }
      if (current.type == EventType.MouseDrag && GUIUtility.hotControl == this.id)
      {
        switch (this.m_DragState)
        {
          ...
          case TimelineControl.DragStates.Playhead:
            ...
            this.Time = this.m_TimeArea.PixelToTime(this.m_TimeStartDrag + this.m_TimeOffset, rect1);
            break;
        }
        current.Use();
        GUI.changed = true;
      ...
      for (int index = 0; index < num4; ++index)
      {
        ...
        float num7 = num6 + 15f;
        if ((double) num6 < (double) x && (double) num7 > 0.0 && this.m_DragState == TimelineControl.DragStates.LeftSelection)
          b = 0.0f;
        GUI.EndGroup();
      ...
      for (int index = 0; index < num8; ++index)
      {
        ...
        float num6 = Mathf.Max(0.0f, num5) - 20f;
        float num7 = num6 + 15f;
        if ((double) num6 < (double) x && (double) num7 > 0.0 && (this.m_DragState == TimelineControl.DragStates.LeftSelection || this.m_DragState == TimelineControl.DragStates.Destination))
          b = 0.0f;
        GUI.EndGroup();
      ...
      if (this.m_DragState == TimelineControl.DragStates.Destination && !flag3)
