...
internal class ParticleSystemCurveEditor
{
  ...
  {
    return (double) Mathf.Abs(c1.r - c2.r) < 0.00999999977648258 && (double) Mathf.Abs(c1.g - c2.g) < 0.00999999977648258 && (double) Mathf.Abs(c1.b - c2.b) < 0.00999999977648258;
  }
  ...
  public void OnGUI(Rect rect)
  {
    ...
    Rect position1 = new Rect(rect.x, rect.y, rect.width, rect.height - 30f);
    Rect position2 = new Rect(rect.x, rect.y + position1.height, rect.width, 30f);
    GUI.Label(position1, GUIContent.none, ParticleSystemCurveEditor.s_Styles.curveEditorBackground);
    if (Event.current.type == EventType.Repaint)
      this.m_CurveEditor.rect = position1;
    ...
    this.DoRemoveSelectedButton(new Rect(position1.x, position1.y, position1.width, 24f));
    this.DoOptimizeCurveButton(new Rect(position1.x, position1.y, position1.width, 24f));
    position2.x += 30f;
    position2.width -= 60f;
    this.PresetCurveButtons(position2, rect);
      ...
      if (addedCurve.m_MaxId == curveID || addedCurve.m_MinId == curveID)
      {
        if (doubleCurve.signedRange == addedCurve.m_SignedRange)
        {
          if (addedCurve.m_MaxId > 0)
            this.SetCurve(this.m_CurveEditor.GetCurveWrapperFromID(addedCurve.m_MaxId), doubleCurve.maxCurve);
          if (addedCurve.m_MinId > 0)
            this.SetCurve(this.m_CurveEditor.GetCurveWrapperFromID(addedCurve.m_MinId), doubleCurve.minCurve);
        }
        else
  ...
  private void InitDoubleCurvePresets()
  {
    ...
    this.m_DoubleCurvePresets = new DoubleCurvePresetsContentsForPopupWindow(this.CreateDoubleCurveFromTopMostCurve(), (Action<DoubleCurve>) (presetDoubleCurve =>
    {
  ...
  {
    bool flag = false;
    Vector2 vector2 = new Vector2(64f, 14f);
    Rect position = new Rect(rect.xMax - 80f - vector2.x, rect.y + (float) (((double) rect.height - (double) vector2.y) * 0.5), vector2.x, vector2.y);
    if (!this.m_CurveEditor.IsDraggingCurveOrRegion())
    {
      ...
      if (selectedCurves.Count > 0)
      {
        ...
        {
          CurveWrapper wrapperFromSelection = this.m_CurveEditor.GetCurveWrapperFromSelection(selectedCurves[index]);
          num += !AnimationUtility.IsValidOptimizedPolynomialCurve(wrapperFromSelection.curve) ? 0 : 1;
        }
        if (selectedCurves.Count != num)
        {
          ...
          if (GUI.Button(position, ParticleSystemCurveEditor.s_Styles.optimizeCurveText))
          {
            ...
            {
              CurveWrapper wrapperFromSelection = this.m_CurveEditor.GetCurveWrapperFromSelection(selectedCurves[index]);
              if (!AnimationUtility.IsValidOptimizedPolynomialCurve(wrapperFromSelection.curve))
              {
                wrapperFromSelection.curve.preWrapMode = WrapMode.Once;
                wrapperFromSelection.curve.postWrapMode = WrapMode.Once;
                wrapperFromSelection.renderer.SetWrap(WrapMode.Once, WrapMode.Once);
                AnimationUtility.ConstrainToPolynomialCurve(wrapperFromSelection.curve);
  ...
  private void DoRemoveSelectedButton(Rect rect)
  {
    ...
    {
      Vector2 vector2 = new Vector2(64f, 14f);
      if (!GUI.Button(new Rect((float) ((double) rect.x + (double) rect.width - (double) vector2.x - 10.0), rect.y + (float) (((double) rect.height - (double) vector2.y) * 0.5), vector2.x, vector2.y), ParticleSystemCurveEditor.s_Styles.removeCurveText))
        return;
  ...
  private void SaveCurve(SerializedProperty prop, CurveWrapper cw)
  {
    ...
    {
      cw.renderer.SetCustomRange(0.0f, 1f);
  ...
  private CurveWrapper CreateCurveWrapper(SerializedProperty curve, int id, int regionId, Color color, bool signedRange, CurveWrapper.GetAxisScalarsCallback getAxisScalarsCallback, CurveWrapper.SetAxisScalarsCallback setAxisScalarsCallback)
  {
    ...
    {
      curveWrapper.renderer.SetCustomRange(0.0f, 0.0f);
  ...
  private CurveWrapper[] CreateCurveWrapperArray()
  {
    ...
    {
      ParticleSystemCurveEditor.CurveData addedCurve = this.m_AddedCurves[index];
      if (addedCurve.m_Visible && (addedCurve.m_Max == null || !addedCurve.m_Max.hasMultipleDifferentValues) && (addedCurve.m_Min == null || !addedCurve.m_Min.hasMultipleDifferentValues))
      {
        int regionId = -1;
  ...
  {
    public GUIStyle curveEditorBackground = (GUIStyle) "CurveEditorBackground";
