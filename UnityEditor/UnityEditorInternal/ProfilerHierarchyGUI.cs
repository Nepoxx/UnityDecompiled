...
namespace UnityEditorInternal
{
  ...
  internal class ProfilerHierarchyGUI
  {
    ...
    private SplitterState m_Splitter = (SplitterState) null;
    ...
    {
      ProfilerProperty profilerProperty1 = this.m_Window.GetRootProfilerProperty(this.sortType);
      bool enterChildren = true;
      string selectedPropertyPath = ProfilerDriver.selectedPropertyPath;
      while (profilerProperty1.Next(enterChildren))
      {
        string propertyPath = profilerProperty1.propertyPath;
        if (propertyPath == selectedPropertyPath)
        {
          ProfilerProperty profilerProperty2 = new ProfilerProperty();
          profilerProperty2.InitializeDetailProperty(profilerProperty1);
          return profilerProperty2;
        }
        if (profilerProperty1.HasChildren)
    ...
    {
      return !ProfilerDriver.enabled || !ProfilerDriver.profileEditor && !EditorApplication.isPlaying || !ProfilerDriver.deepProfiling;
    }
    ...
    public void DoGUI(ProfilerProperty property, string searchString, bool expandAll)
    {
      ...
      this.UnselectIfClickedOnEmptyArea(this.DrawProfilingData(property, searchString, controlId));
