...
namespace UnityEditorInternal
{
  public static class SpriteEditorHandles
  {
      ...
      switch (current.GetTypeForControl(id))
      {
        ...
        case EventType.MouseDrag:
          if (GUIUtility.hotControl == id)
          {
            ...
            pos = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) SpriteEditorHandles.s_CurrentMousePosition);
        ...
        case EventType.KeyDown:
          ...
          {
            pos = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) (SpriteEditorHandles.s_DragStartScreenPosition - SpriteEditorHandles.s_DragScreenOffset));
            GUIUtility.hotControl = 0;
            GUI.changed = true;
            current.Use();
            break;
      ...
      switch (current.GetTypeForControl(controlId))
      {
        ...
        case EventType.MouseDrag:
          if (GUIUtility.hotControl == controlId)
          {
            SpriteEditorHandles.s_CurrentMousePosition += current.delta;
            Vector2 vector2_2 = pos;
            Vector3 vector3 = Handles.inverseMatrix.MultiplyPoint((Vector3) (SpriteEditorHandles.s_CurrentMousePosition - SpriteEditorHandles.s_DragScreenOffset));
    ...
    {
      int controlId = GUIUtility.GetControlID(nameof (SliderRect).GetHashCode(), FocusType.Keyboard);
      Event current = Event.current;
      ...
      switch (current.GetTypeForControl(controlId))
      {
        ...
        case EventType.MouseDrag:
          if (GUIUtility.hotControl == controlId)
          {
            ...
            pos.center = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) (SpriteEditorHandles.s_CurrentMousePosition - SpriteEditorHandles.s_DragScreenOffset));
        ...
        case EventType.KeyDown:
          if (GUIUtility.hotControl == controlId && current.keyCode == KeyCode.Escape)
          {
            pos.center = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) (SpriteEditorHandles.s_DragStartScreenPosition - SpriteEditorHandles.s_DragScreenOffset));
            GUIUtility.hotControl = 0;
            GUI.changed = true;
            current.Use();
            break;
          ...
          break;
        case EventType.Repaint:
          Vector2 vector2_1 = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) new Vector2(pos.xMin, pos.yMin));
          Vector2 vector2_2 = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) new Vector2(pos.xMax, pos.yMax));
      ...
      switch (current.GetTypeForControl(rectSelectionId))
      {
        case EventType.MouseDown:
          if (current.button == 0)
          {
            ...
            Vector2 vector2 = (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) mousePosition);
            ...
            SpriteEditorHandles.s_DragStartScreenPosition = (Vector2) Handles.matrix.MultiplyPoint((Vector3) vector2);
        ...
        case EventType.MouseUp:
          if (GUIUtility.hotControl == rectSelectionId && current.button == 0)
          {
            ...
            {
              rect = SpriteEditorHandles.GetCurrentRect(false, textureArea, SpriteEditorHandles.s_DragStartScreenPosition, SpriteEditorHandles.s_CurrentMousePosition);
              GUI.changed = true;
              current.Use();
            }
            GUIUtility.hotControl = 0;
    ...
    {
      return SpriteEditorHandles.RectCreator(new Rect(0.0f, 0.0f, textureWidth, textureHeight), rectStyle);
    }

    private static bool ValidRect(Vector2 startPoint, Vector2 endPoint)
    {
      return (double) Mathf.Abs((endPoint - startPoint).x) > 5.0 && (double) Mathf.Abs((endPoint - startPoint).y) > 5.0;
    }

    private static Rect GetCurrentRect(bool screenSpace, Rect clampArea, Vector2 startPoint, Vector2 endPoint)
    {
      Rect rect = SpriteEditorUtility.ClampedRect(SpriteEditorUtility.RoundToInt(EditorGUIExt.FromToRect((Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) startPoint), (Vector2) Handles.inverseMatrix.MultiplyPoint((Vector3) endPoint))), clampArea, false);
      if (screenSpace)
      {
