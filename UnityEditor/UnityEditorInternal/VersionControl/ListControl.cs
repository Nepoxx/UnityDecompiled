...
namespace UnityEditorInternal.VersionControl
{
  ...
  public class ListControl
  {
    ...
    private ListItem active = (ListItem) null;
    private List<ListItem> visibleList = new List<ListItem>();
    private Texture2D blueTex = (Texture2D) null;
    private Texture2D greyTex = (Texture2D) null;
    private Texture2D yellowTex = (Texture2D) null;
    private Dictionary<string, ListItem> pathSearch = new Dictionary<string, ListItem>();
    private Texture2D defaultIcon = (Texture2D) null;
    private bool readOnly = false;
    private bool scrollVisible = false;
    private string menuFolder = (string) null;
    private string menuDefault = (string) null;
    private bool dragAcceptOnly = false;
    private ListItem dragTarget = (ListItem) null;
    ...
    private ListItem singleSelect = (ListItem) null;
    ...
    public AssetList SelectedAssets
    {
      get
      {
        ...
        foreach (KeyValuePair<string, ListItem> select in this.selectList)
        {
          if (select.Value.Item is Asset)
            assetList.Add(select.Value.Item as Asset);
        }
        return assetList;
      }
    }
    ...
    public ChangeSets SelectedChangeSets
    {
      get
      {
        ...
        foreach (KeyValuePair<string, ListItem> select in this.selectList)
        {
          if (select.Value != null && select.Value.Item is ChangeSet)
            changeSets.Add(select.Value.Item as ChangeSet);
        }
        return changeSets;
      }
    }
    ...
    {
      float y = area.y;
      foreach (ListItem visible in this.visibleList)
      {
        float x = area.x + (float) ((visible.Indent - 1) * 18);
        bool selected = !this.readOnly && this.IsSelected(visible);
        if (visible.Parent != null && visible.Parent.Parent != null && visible.Parent.Parent.Parent == null)
          x -= 16f;
        this.DrawItem(visible, area, x, y, focus, selected);
        y += 16f;
      }
    }
    ...
    private bool IsSelectedAsset()
    {
      foreach (KeyValuePair<string, ListItem> select in this.selectList)
      {
        if (select.Value != null && select.Value.Asset != null)
          return true;
      }
      return false;
    }
    ...
    private void SelectedRefresh()
    {
      ...
      foreach (KeyValuePair<string, ListItem> select in this.selectList)
        dictionary[select.Key] = this.PathSearchFind(select.Key);
    ...
    private ListItem SelectedCurrentIn(ListItem root)
    {
      foreach (KeyValuePair<string, ListItem> select in this.selectList)
      {
        if (select.Value.IsChildOf(root))
          return select.Value;
      }
      return (ListItem) null;
    }
