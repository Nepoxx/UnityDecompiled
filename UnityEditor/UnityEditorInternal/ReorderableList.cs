        ...
        if (this.IsDragging() && Event.current.type == EventType.Repaint)
        {
          ...
          for (int index = 0; index < this.m_NonDragTargetIndices.Count; ++index)
          {
            if (this.m_NonDragTargetIndices[index] != -1)
            {
              ...
              {
                rect1.y = listRect.y + this.GetElementYOffset(this.m_NonDragTargetIndices[index], this.m_ActiveElement);
                if (flag)
    ...
    public class Defaults
    {
      ...
      public void DrawFooter(Rect rect, ReorderableList list)
      {
        ...
        if (list.displayAdd)
        {
          ...
          {
            if (GUI.Button(rect1, list.onAddDropdownCallback == null ? this.iconToolbarPlus : this.iconToolbarPlusMore, this.preButton))
            {
