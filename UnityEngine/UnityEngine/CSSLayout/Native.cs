...
// Type: UnityEngine.CSSLayout.Native
  ...
  internal static class Native
  {
    ...
    public static void CSSNodeFree(IntPtr cssNode)
    {
      if (cssNode == IntPtr.Zero)
        return;
      Native.CSSNodeSetMeasureFunc(cssNode, (CSSMeasureFunc) null);
    ...
    public static void CSSNodeSetMeasureFunc(IntPtr node, CSSMeasureFunc measureFunc)
    {
      if (measureFunc != null)
      {
        Native.s_MeasureFunctions[node] = new WeakReference((object) measureFunc);
        CSSLayoutCallbacks.RegisterWrapper(node);
      }
      else
      {
        if (!Native.s_MeasureFunctions.ContainsKey(node))
          return;
    ...
    public static CSSMeasureFunc CSSNodeGetMeasureFunc(IntPtr node)
    {
      ...
      if (Native.s_MeasureFunctions.TryGetValue(node, out weakReference) && weakReference.IsAlive)
        return weakReference.Target as CSSMeasureFunc;
      return (CSSMeasureFunc) null;
    }

    [RequiredByNativeCode]
    public static unsafe void CSSNodeMeasureInvoke(IntPtr node, float width, CSSMeasureMode widthMode, float height, CSSMeasureMode heightMode, IntPtr returnValueAddress)
    {
      CSSMeasureFunc measureFunc = Native.CSSNodeGetMeasureFunc(node);
      if (measureFunc == null)
        return;
      *(CSSSize*) (void*) returnValueAddress = measureFunc(node, width, widthMode, height, heightMode);
    }
